...
public class AccountOperatorUserAuthorityQueryOrchestrator {
    ...
    public AccountOperatorsUsersAuthoritiesInfoDTO getAccountOperatorsUsersAuthoritiesInfo(AppUser authUser, Long accountId) {
        ...
        List<OperatorUserInfoDTO> operatorUsersInfo = operatorUserInfoService.getOperatorUsersInfo(userIds);

        return getAccountOperatorUserAuthoritiesInfoDTO(accountAuthorities, contactTypes, operatorUsersInfo);
    }

    private AccountOperatorsUsersAuthoritiesInfoDTO getAccountOperatorUserAuthoritiesInfoDTO(UserAuthoritiesDTO accountAuthorities,
            Map<AccountContactType, String> contactTypes, List<OperatorUserInfoDTO> operatorUsersInfo) {
        ...
        accountAuthorities.getAuthorities().stream()
                .map(authority ->
                        userAuthorityInfoMapper.toUserAuthorityInfo(
                                authority,
                                operatorUsersInfo.stream()
                                        .filter(info -> info.getId().equals(authority.getUserId()))
                                        .findFirst()
                                        .orElse(new OperatorUserInfoDTO())))
