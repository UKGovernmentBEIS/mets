...
public class NEROfficialNoticeService {
    ...
    public void sendOfficialNotice(String requestId) {
        ...
        if (requestPayload.getRegulatorReviewOutcome() == null) {
            throw new BusinessException(
                    ...
                    requestPayload,
                    NERViolation.INVALID_REGULATOR_REVIEW_OUTCOME.getMessage());
        }
        final String templateName = requestPayload.getRegulatorReviewOutcome().getOpinion()
                == NERReviewOpinion.PROCEED_TO_AUTHORITY
                ? PmrvNotificationTemplateName.NER_COMPLETED.getName()
                : PmrvNotificationTemplateName.NER_WITHDRAWN.getName();
