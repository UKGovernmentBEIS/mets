    ...
    public List<AccountUserContact> findAccountUserContacts(EntityManager entityManager) {
        return entityManager.createNativeQuery(
                "select auth.user_id as \"userId\", role.name as \"role\",  account.emitter_id as \"accountId\", account.name as \"accountName\", " +
                    "account.type as \"accountType\", acc_av.status as \"accountStatus\", le.name as \"legalEntityName\", auth.status as \"authorityStatus\",\n" +
            ...
            .setReadOnly(true)
            .setResultTransformer(Transformers.aliasToBean(AccountUserContact.class)) //https://vladmihalcea.com/hibernate-resulttransformer/
            .getResultList();
    }
}
