    ...
    private static final String QUERY_BASE = """
        ...
        mxPVer as (
            ...
            select * from (values
                ...
                ('FCD57438-3638-4DA2-8B93-A3F600B2A3F9','Marston’s House','Brewery Road','Wolverhampton','WV1 4JT','GB'),
                ('666F7DDB-6801-466B-A090-A7FC0076F537','23 Lower Belgrave Street',null,'London','SW1W 0NR','GB'),
                ('5BF4E3D1-1C62-4D79-B045-9FA600CE5D76','Charing Cross Hospital - Estates Department - CBRE','Fulham Palace Road','London','W6 8RF','GB'),
                ('D5ED296F-6A62-4244-B258-A0A601053618','Second Floor, Arena Court','Crown Lane','Maidenhead','SL6 8QZ','GB'),
                ('745565BC-DCE1-4D27-97B3-9FA600CE5D76','Room 1.02, First Floor','Cobbett House','Oxford Rd','M13 9WL','GB'),
                ('D1B04E7F-40F6-4B7C-A960-A0C400ADB670','Anick Road',null,'Hexham','NE46 4JS','GB')
            ...
            select o.fldOperatorID as operatorId,
                   e.fldEmitterDisplayId as emitterDisplayId,
                   e.fldName as emitterName,
                   ie.fldEmitterID as emitterId,
                   ca.fldName as competent_authority,
                   nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_operator_legal_status)[1]', 'NVARCHAR(max)')), '') as 'type',
                   o.fldName as name,
                   ...
                   case when c.operatorId is not null then c.line1 else
                     ...
                     when 'Company / Corporate Body' THEN nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_company_registered_office_address-Address_line_1)[1]', 'NVARCHAR(max)')), '')
                     when 'Partnership' THEN nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_partnership_address-Address_line_1)[1]', 'NVARCHAR(max)')), '')
                     else NULL end
                   end as line1,
                   case when c.operatorId is not null then c.line2 else
                     case F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_operator_legal_status)[1]', 'NVARCHAR(max)')
                     when 'Company / Corporate Body' THEN nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_company_registered_office_address-Address_line_2)[1]', 'NVARCHAR(max)')), '')
                     when 'Partnership' THEN nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_partnership_address-Address_line_2)[1]', 'NVARCHAR(max)')), '')
                     else NULL end
                   end as line2,
                   case when c.operatorId is not null then c.city else
                     case F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_operator_legal_status)[1]', 'NVARCHAR(max)')
                     when 'Company / Corporate Body' THEN nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_company_registered_office_address-City)[1]', 'NVARCHAR(max)')), '')
                     when 'Partnership' THEN nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_partnership_address-City)[1]', 'NVARCHAR(max)')), '')
                     else NULL end
                   end as city,
                   case when c.operatorId is not null then c.postcode else
                     case F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_operator_legal_status)[1]', 'NVARCHAR(max)')
                     when 'Company / Corporate Body' THEN nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_company_registered_office_address-Postcode)[1]', 'NVARCHAR(max)')), '')
                     when 'Partnership' THEN nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_partnership_address-Postcode)[1]', 'NVARCHAR(max)')), '')
                     else NULL end
                   end as postcode,
                   case when c.operatorId is not null then c.country else null end as country,
                   nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_company_belongs_to_holding_company)[1]', 'NVARCHAR(max)')), '') belongsToHoldingCompany,
                   nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_company_holding_company_name)[1]', 'NVARCHAR(max)')), '') holdingCompanyName,
                   nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_company_holding_company_company_registration_number)[1]', 'NVARCHAR(max)')), '') holdingCompanyRegistrationNumber,
                   nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_company_holding_company_address-Address_line_1)[1]', 'NVARCHAR(max)')), '') holdingCompanyAddressLine1,
                   nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_company_holding_company_address-Address_line_2)[1]', 'NVARCHAR(max)')), '') holdingCompanyAddressLine2, 
                   nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_company_holding_company_address-Postcode)[1]', 'NVARCHAR(max)')), '') holdingCompanyAddressPostcode,
                   nullif(trim(F.fldSubmittedXML.value('(fd:formdata/fielddata/Od_company_holding_company_address-City)[1]', 'NVARCHAR(max)')), '') holdingCompanyAddressCity
              from tblEmitter e
              ...
              join tblOperator o             on e.fldOperatorID = o.fldOperatorID
              ...
              join tblInstallationEmitter ie on ie.fldEmitterID = e.fldEmitterID
        ...
        select operatorId,
               ...
               holdingCompanyName,
               holdingCompanyRegistrationNumber,
               holdingCompanyAddressLine1,
               holdingCompanyAddressLine2,
               holdingCompanyAddressPostcode,
               holdingCompanyAddressCity
