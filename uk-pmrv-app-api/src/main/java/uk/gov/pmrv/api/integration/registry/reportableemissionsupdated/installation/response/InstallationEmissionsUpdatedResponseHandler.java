...
public class InstallationEmissionsUpdatedResponseHandler {
        ...
        if (event.getOutcome() == RegistryResponseStatus.ERROR) {
            ...
            if (!ObjectUtils.isEmpty(errorsForMail)) {
                if (errorsForMail.containsKey(RegistryResponseErrorCode.ERROR_0803.getCode())) {
                    notifyRegulator(event, correlationId, Map.of(
                ...
                log.info(RESPONSE_LOG_FORMAT,
                    ...
                    "Failed to process an installation request and notified regulator with errors " + event);
            } else {
                log.info(RESPONSE_LOG_FORMAT,
                    SERVICE_KEY,
                    event.getEvent().getRegistryId(),
                    INTEGRATION_POINT_KEY,
                    "Failed to process an installation request, but received unknown error(s) " + event);
            }
        }
    }
