...
public class EmpAdditionalDocumentsCorsiaSectionMigrationService 
	...
	private static final String QUERY_BASE = "with XMLNAMESPACES (\r\n"
    		...
    		+ ")\r\n"
    		+ "select e.fldEmitterID, m.fldEmitterDisplayID, lnk.uploadedFileName, lnk.storedFileName\r\n"
    		+ "  from tblEmitter e\r\n"
    ...
    @Override
    public Map<String, EmpAdditionalDocuments> querySection(List<String> accountIds) {
	...
	private Map<String, List<EtsFileAttachment>>  executeQuery(String query, List<String> accountIds) {
        ...
        return etsAdditionalDocuments
        		.stream()
                .filter(etsFileAttachment -> MigrationConstants.ALLOWED_FILE_TYPES.contains(
                		etsFileAttachment.getUploadedFileName().substring(
                				etsFileAttachment.getUploadedFileName().lastIndexOf(".")).toLowerCase()))
