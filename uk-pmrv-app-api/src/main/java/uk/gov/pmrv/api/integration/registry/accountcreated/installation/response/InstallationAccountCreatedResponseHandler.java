...
public class InstallationAccountCreatedResponseHandler {
        ...
        if (IntegrationEventOutcome.ERROR.equals(accountOpeningEventOutcome.getOutcome())) {
            ...
            if (!ObjectUtils.isEmpty(errors)) {
                ...
                log.info(RESPONSE_LOG_FORMAT,
                    ...
                    "Failed to process an installation request and notified regulator with errors " + accountOpeningEventOutcome);
            } else {
                log.info(RESPONSE_LOG_FORMAT,
                    NotifyRegistryUtils.INSTALLATION_SERVICE_KEY,
                    accountOpeningEventOutcome.getEvent().getAccountDetails().getEmitterId(),
                    NotifyRegistryUtils.ACCOUNT_CREATED_INTEGRATION_POINT_KEY,
                    "Failed to process an installation request, but received unknown error(s) " + accountOpeningEventOutcome);
            }
        }
    }
