...
public class InstallationAccountCreatedSendToRegistryProducer {

    @Value("${kafka.installation.account-created-request.topic}")
    private String topicName;

    private final KafkaTemplate<String, AccountOpeningEvent> installationAccountCreatedKafkaTemplate;

    @Transactional
    public void produce(AccountOpeningEvent accountOpeningEvent) {
        try {
            installationAccountCreatedKafkaTemplate.send(topicName, String.valueOf(accountOpeningEvent.getAccountDetails().getEmitterId()), accountOpeningEvent);
        } catch (Exception e) {
