...
@RestController
@RequestMapping(path = "/v1.0/{accountType}/mireports/system")
@RequiredArgsConstructor
@Tag(name = "MiReports")
@Validated
public class MiReportSystemController {

    private final PmrvMiReportSystemService pmrvMiReportSystemService;
    ...
    @ApiResponse(responseCode = "200", description = OK, content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, array = @ArraySchema(schema = @Schema(implementation = MiReportSystemSearchResult.class))))
    @ApiResponse(responseCode = "500", description = INTERNAL_SERVER_ERROR, content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class))})
    @AuthorizedRole(roleType = {REGULATOR})
    public ResponseEntity<List<MiReportSystemSearchResult>> getCurrentUserMiReports(
        ...
        List<MiReportSystemSearchResult> results =
        	pmrvMiReportSystemService.findByCompetentAuthorityAndAccountType(appUser.getCompetentAuthority(), accountType);
        return ResponseEntity.ok(results);
    }

    @PostMapping
    @Operation(summary = "Generates the report identified by the provided report type")
    @ApiResponse(responseCode = "200", description = OK, content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = MiReportSystemResult.class)))
    ...
    public ResponseEntity<MiReportSystemResult> generateReport(@Parameter(hidden = true) AppUser appUser,
                                                         ...
                                                         @Valid
                                                         MiReportSystemParams reportParams) {
        MiReportSystemResult reportResult = pmrvMiReportSystemService.generateReport(appUser.getCompetentAuthority(), accountType, reportParams);
