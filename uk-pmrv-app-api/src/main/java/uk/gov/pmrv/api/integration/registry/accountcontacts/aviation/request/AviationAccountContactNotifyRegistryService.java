...
public class AviationAccountContactNotifyRegistryService {

    private final AccountContactRegistryIntegrationUtilityService utilityService;
    private final AviationAccountContactRegistryProducer registryProducer;

    @Transactional
    public void notifyRegistry(Account account) {

        if(ObjectUtils.isEmpty(account.getRegistryId())) {
            log.error(REQUEST_LOG_FORMAT, NotifyRegistryUtils.AVIATION_SERVICE_KEY, account.getId(),
        ...
        log.info(REQUEST_LOG_FORMAT, NotifyRegistryUtils.AVIATION_SERVICE_KEY, account.getId(),
