...
@Component
@AllArgsConstructor
@KafkaListener(topics = "${kafka.aviation.account-contact-response.topic}",
        containerFactory = "aviationAccountContactKafkaListenerContainerFactory")
@ConditionalOnProperty(name = "registry.integration.account.contact.enabled", havingValue = "true", matchIfMissing = false)
public class AviationAccountContactResponseEventListener {
    ...
    public void handle(@Payload MetsContactsEventOutcome event, @Header(KafkaConstants.CORRELATION_ID_HEADER) String correlationId) {
        handler.handleResponse(event, correlationId, NotifyRegistryUtils.AVIATION_SERVICE_KEY);
    }
