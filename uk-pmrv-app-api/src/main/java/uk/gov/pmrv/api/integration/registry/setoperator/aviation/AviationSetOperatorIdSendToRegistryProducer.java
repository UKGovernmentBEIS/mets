...
public class AviationSetOperatorIdSendToRegistryProducer {

    @Value("${kafka.aviation.operator-identifier-response.topic}")
    private String topicName;

    private final KafkaTemplate<String, OperatorUpdateEventOutcome> aviationSetOperatorIdKafkaTemplate;

    @Transactional
    public void produce(OperatorUpdateEventOutcome eventOutcome) {
        try {
            aviationSetOperatorIdKafkaTemplate.send(topicName, String.valueOf(eventOutcome.getEvent().getEmitterId()), eventOutcome);
        } catch (Exception e) {
