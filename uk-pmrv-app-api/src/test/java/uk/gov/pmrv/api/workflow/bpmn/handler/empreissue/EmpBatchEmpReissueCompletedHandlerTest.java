...
class EmpBatchEmpReissueCompletedHandlerTest {
    ...
    void execute() throws Exception {
        String requestId = "1";
        String reissueRequestId = "2";
        Long accountId = 3L;
        boolean reissueSucceeded = true;
        Integer numberOfAccountsCompleted = 0;
        when(execution.getVariable(BpmnProcessConstants.REQUEST_ID)).thenReturn(requestId);
        when(execution.getVariable(BpmnProcessConstants.REISSUE_REQUEST_ID)).thenReturn(reissueRequestId);
        ...
        verify(service, times(1)).reissueCompleted(requestId, accountId, reissueRequestId, reissueSucceeded);
    }
