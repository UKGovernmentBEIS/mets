...
class FileDocumentTemplateControllerTest {
    ...
    private static final String CONTROLLER_PATH = "/v1.0/file-document-templates";

    private MockMvc mockMvc;

    @InjectMocks
    private FileDocumentTemplateController fileDocumentTemplateController;

    @Mock
    private FileDocumentTemplateTokenService fileDocumenTemplateTokenService;
    ...
    @BeforeEach
    void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(fileDocumentTemplateController)
    ...
    void getFileDocument() throws Exception {
        ...
        when(fileDocumenTemplateTokenService.getFileDTOByToken(token)).thenReturn(file);
