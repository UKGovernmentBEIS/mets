...
class AlrApplicationAddAcceptedWithCorrectionsRequestActionHandlerFlowableTest {
    ...
    @InjectMocks
    private AlrApplicationAddAcceptedWithCorrectionsRequestActionHandlerFlowable handler;

    @Test
    void execute_shouldAddAcceptedWithCorrectionsRequestAction() {
        String requestId = "REQ-12345";
        when(execution.getVariable(BpmnProcessConstants.REQUEST_ID)).thenReturn(requestId);

        handler.execute(execution);
        verify(alrAuthorityResponseService)
                .addSubmittedRequestAction(requestId, RequestActionType.ALR_APPLICATION_ACCEPTED_WITH_CORRECTIONS);
    }
    ...
    void execute_shouldPassNullRequestId_whenExecutionVariableMissing() {
        ...
        handler.execute(execution);
        verify(alrAuthorityResponseService)
                .addSubmittedRequestAction(null, RequestActionType.ALR_APPLICATION_ACCEPTED_WITH_CORRECTIONS);
    }
}
