...
import uk.gov.pmrv.api.workflow.request.flow.installation.permitvariation.submitregulatorled.handler.PermitVariationReviewPeerDecisionRegulatorLedActionHandler;

@ExtendWith(MockitoExtension.class)
class PermitVariationReviewPeerDecisionRegulatorLedActionHandlerTest {
	
	@InjectMocks
    private PermitVariationReviewPeerDecisionRegulatorLedActionHandler handler;
    ...
    void process() {
        ...
        final PeerReviewDecisionRequestTaskActionPayload payload =
            PeerReviewDecisionRequestTaskActionPayload.builder()
                .payloadType(RequestTaskActionPayloadType.PERMIT_VARIATION_REVIEW_SUBMIT_PEER_REVIEW_DECISION_PAYLOAD)
                .decision(decision)
                .build();
        ...
        verify(requestService, times(1)).addActionToRequest(
            ...
            eq(RequestActionType.PERMIT_VARIATION_APPLICATION_PEER_REVIEWER_REJECTED),
            eq(userId));
        ...
        assertThat(captorValue.getPayloadType()).isEqualTo(RequestActionPayloadType.PERMIT_VARIATION_PEER_REVIEW_DECISION_REGULATOR_LED_SUBMITTED_PAYLOAD);
    ...
    void getRequestTaskTypes() {
        assertThat(handler.getTypes()).containsExactly(RequestTaskActionType.PERMIT_VARIATION_REVIEW_SUBMIT_PEER_REVIEW_DECISION_REGULATOR_LED);
    }
}
