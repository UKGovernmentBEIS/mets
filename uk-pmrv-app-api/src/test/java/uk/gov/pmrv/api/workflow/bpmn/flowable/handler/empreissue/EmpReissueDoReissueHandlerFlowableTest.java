...
class EmpReissueDoReissueHandlerFlowableTest {

    @InjectMocks
    private EmpReissueDoReissueHandlerFlowable cut;
    ...
    @Test
    void execute_error_occurred() {
        String requestId = "1";
        when(execution.getVariable(BpmnProcessConstants.REQUEST_ID)).thenReturn(requestId);
        doThrow(RuntimeException.class).when(service).doReissue(requestId);
