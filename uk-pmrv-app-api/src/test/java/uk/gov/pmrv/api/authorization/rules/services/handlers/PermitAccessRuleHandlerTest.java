...
class PermitAccessRuleHandlerTest {
    ...
    void single_rule() {
        AuthorizationRuleScopePermission authorizationRulePermissionScope1 =
            AuthorizationRuleScopePermission.builder()
                .resourceType("PERMIT")
                .handler("permitAccessHandler")
                .build();

        when(permitAuthorityInfoProvider.getPermitAccountById("UK-E-IN-00001")).thenReturn(1l);

        AuthorizationCriteria authorizationCriteria = AuthorizationCriteria.builder()
            .requestResources(Map.of(ResourceType.ACCOUNT, "1"))
            .build();

        permitAccessRuleHandler.evaluateRules(Set.of(authorizationRulePermissionScope1), REGULATOR_USER, "UK-E-IN-00001");
        verify(appAuthorizationService, times(1)).authorize(REGULATOR_USER, authorizationCriteria);

        permitAccessRuleHandler.evaluateRules(Set.of(authorizationRulePermissionScope1), OPERATOR_USER, "UK-E-IN-00001");
        verify(appAuthorizationService, times(1)).authorize(OPERATOR_USER, authorizationCriteria);

        permitAccessRuleHandler.evaluateRules(Set.of(authorizationRulePermissionScope1), VERIFIER_USER, "UK-E-IN-00001");
