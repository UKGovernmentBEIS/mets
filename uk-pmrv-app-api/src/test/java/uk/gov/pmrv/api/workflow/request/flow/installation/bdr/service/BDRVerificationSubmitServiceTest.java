...
public class BDRVerificationSubmitServiceTest {

    @InjectMocks
    private BDRVerificationSubmitService service;
    ...
    void applySaveAction() {
         ...
         final BDRRequestPayload aerRequestPayload = BDRRequestPayload.builder()
        ...
        final BDRVerificationData verificationData = BDRVerificationData.builder()
        ...
        final BDRApplicationVerificationSaveRequestTaskActionPayload actionPayload =
                BDRApplicationVerificationSaveRequestTaskActionPayload.builder()
        ...
        BDRApplicationVerificationSubmitRequestTaskPayload payloadSaved =
        ...
        assertThat(((BDRRequestPayload) request.getPayload()).getVerificationReport())
                    .isEqualTo(taskPayload.getVerificationReport());

        assertThat(((BDRRequestPayload) request.getPayload()).isVerificationPerformed()).isFalse();

        assertThat(((BDRRequestPayload) request.getPayload()).getVerificationReport().getVerificationBodyId())
                    .isEqualTo(verificationBodyId);

        assertThat(((BDRRequestPayload) request.getPayload()).getVerificationSectionsCompleted())
                    .containsExactlyEntriesOf(actionPayload.getVerificationSectionsCompleted());

        assertThat(((BDRRequestPayload) request.getPayload()).getVerificationAttachments())
    ...
    void sendToOperator() {
        ...
        final BDR bdr = BDR.builder().build();
        final BDRVerificationReport verificationReport = BDRVerificationReport.builder().build();

        Request request = Request.builder()
                .payload(BDRRequestPayload.builder().bdr(bdr).payloadType(RequestPayloadType.BDR_REQUEST_PAYLOAD).build())
                .build();
        ...
        BDRApplicationVerificationSubmittedRequestActionPayload actionPayload =
                BDRApplicationVerificationSubmittedRequestActionPayload.builder()
                        .payloadType(RequestActionPayloadType.BDR_APPLICATION_VERIFICATION_SUBMITTED_PAYLOAD)
                        .bdr(bdr)
                        .verificationReport(verificationReport)
        ...
        verify(bdrValidationService, times(1))
        ...
        verify(requestService, times(1)).addActionToRequest(requestTask.getRequest(),
                actionPayload, RequestActionType.BDR_APPLICATION_VERIFICATION_SUBMITTED, appUser.getUserId());
