...
class DoalSubmitApplicationProceedToAuthorityActionHandlerTest {

    @InjectMocks
    private DoalSubmitApplicationProceedToAuthorityActionHandler handler;
    ...
    void doProcess() {
        ...
        // Invoke
        handler.process(requestTaskId, RequestTaskActionType.DOAL_PROCEED_TO_AUTHORITY_APPLICATION, user, payload);

        // Verify
        verify(requestTaskService, times(1)).findTaskById(requestTaskId);
        verify(doalProceedToAuthorityValidator, times(1)).validateComplete(requestTask);
    ...
    @Test
    void getTypes() {
        assertThat(handler.getTypes()).containsExactly(RequestTaskActionType.DOAL_PROCEED_TO_AUTHORITY_APPLICATION);
    }
}
