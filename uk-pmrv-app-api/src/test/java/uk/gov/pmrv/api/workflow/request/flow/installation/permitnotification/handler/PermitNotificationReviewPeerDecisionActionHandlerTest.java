...
class PermitNotificationReviewPeerDecisionActionHandlerTest {

    @InjectMocks
    private PermitNotificationReviewPeerDecisionActionHandler handler;
    ...
    void process() {
        ...
        final PeerReviewDecisionRequestTaskActionPayload payload =
                PeerReviewDecisionRequestTaskActionPayload.builder()
                        .payloadType(RequestTaskActionPayloadType.PERMIT_NOTIFICATION_REVIEW_SUBMIT_PEER_REVIEW_DECISION_PAYLOAD)
                        .decision(decision)
                        .build();
        ...
        handler.process(requestTask.getId(),
                RequestTaskActionType.PERMIT_NOTIFICATION_REVIEW_SUBMIT_PEER_REVIEW_DECISION,
                pmrvUser,
                payload);
        ...
        verify(requestService, times(1)).addActionToRequest(
                ...
                eq(RequestActionType.PERMIT_NOTIFICATION_APPLICATION_PEER_REVIEWER_REJECTED),
                eq(userId));
        ...
        assertThat(captorValue.getPayloadType()).isEqualTo(RequestActionPayloadType.PERMIT_NOTIFICATION_PEER_REVIEW_DECISION_SUBMITTED_PAYLOAD);
    ...
    void getRequestTaskTypes() {
        assertThat(handler.getTypes()).containsExactly(RequestTaskActionType.PERMIT_NOTIFICATION_REVIEW_SUBMIT_PEER_REVIEW_DECISION);
    }
}
