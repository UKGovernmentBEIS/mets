...
public class ALRSubmitPeerReviewDecisionActionHandlerTest {

    @InjectMocks
    private ALRSubmitPeerReviewDecisionActionHandler handler;
    ...
    void process() {
        ...
        PeerReviewDecisionRequestTaskActionPayload payload =
                PeerReviewDecisionRequestTaskActionPayload.builder()
                        .payloadType(RequestTaskActionPayloadType.ALR_SUBMIT_PEER_REVIEW_DECISION_PAYLOAD)
                        .decision(decision)
                        .build();
        ...
        verify(requestService, times(1)).addActionToRequest(
                ...
                eq(RequestActionType.ALR_APPLICATION_PEER_REVIEW_REJECTED),
                eq(userId));
        ...
        assertThat(captorValue.getPayloadType()).isEqualTo(RequestActionPayloadType.ALR_APPLICATION_PEER_REVIEW_DECISION_SUBMITTED_PAYLOAD);

        verify(workflowService, times(1)).completeTask(processTaskId, Map.of(
                BpmnProcessConstants.ALR_OUTCOME,
                ALROutcome.SUBMITTED_TO_REGULATOR));
    }

    @Test
    void getRequestTaskTypes() {
        assertThat(handler.getTypes()).containsExactly(RequestTaskActionType.ALR_SUBMIT_PEER_REVIEW_DECISION);
    }
}
