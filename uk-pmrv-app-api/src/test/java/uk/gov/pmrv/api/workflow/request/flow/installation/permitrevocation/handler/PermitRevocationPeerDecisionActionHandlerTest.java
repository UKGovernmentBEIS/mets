...
class PermitRevocationPeerDecisionActionHandlerTest {

    @InjectMocks
    private PermitRevocationPeerDecisionActionHandler handler;
    ...
    void process() {
        ...
        final PeerReviewDecisionRequestTaskActionPayload payload =
            PeerReviewDecisionRequestTaskActionPayload.builder()
                .payloadType(RequestTaskActionPayloadType.PERMIT_REVOCATION_SUBMIT_PEER_REVIEW_DECISION_PAYLOAD)
                .decision(decision)
                .build();
        ...
        handler.process(requestTask.getId(),
            RequestTaskActionType.PERMIT_REVOCATION_SUBMIT_PEER_REVIEW_DECISION,
            appUser,
            payload);
        ...
        verify(requestService, times(1)).addActionToRequest(
            ...
            eq(RequestActionType.PERMIT_REVOCATION_APPLICATION_PEER_REVIEWER_REJECTED),
            eq(userId));
        ...
        assertThat(captorValue.getPayloadType()).isEqualTo(RequestActionPayloadType.PERMIT_REVOCATION_PEER_REVIEW_DECISION_SUBMITTED_PAYLOAD);
    ...
    void getRequestTaskTypes() {
        assertThat(handler.getTypes()).containsExactly(RequestTaskActionType.PERMIT_REVOCATION_SUBMIT_PEER_REVIEW_DECISION);
    }
}
