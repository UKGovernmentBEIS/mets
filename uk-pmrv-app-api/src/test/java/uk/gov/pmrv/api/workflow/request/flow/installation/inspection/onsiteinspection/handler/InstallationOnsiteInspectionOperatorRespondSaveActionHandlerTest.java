...
public class InstallationOnsiteInspectionOperatorRespondSaveActionHandlerTest {

    @InjectMocks
    private InstallationOnsiteInspectionOperatorRespondSaveActionHandler handler;
    ...
    void process(){
        ...
        handler.process(requestTask.getId(), RequestTaskActionType.INSTALLATION_ONSITE_INSPECTION_OPERATOR_RESPOND_SAVE, appUser,
                taskActionPayload);

        verify(installationOnsiteInspectionOperatorRespondService, times(1)).applySaveAction(requestTask, taskActionPayload);
    }

    @Test
    void getTypes(){
        assertThat(handler.getTypes())
                .containsExactlyInAnyOrder(RequestTaskActionType.INSTALLATION_ONSITE_INSPECTION_OPERATOR_RESPOND_SAVE);
    }
}