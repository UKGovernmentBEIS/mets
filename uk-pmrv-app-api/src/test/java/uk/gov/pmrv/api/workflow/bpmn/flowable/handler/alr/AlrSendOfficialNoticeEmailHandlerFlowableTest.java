...
class AlrSendOfficialNoticeEmailHandlerFlowableTest {
    ...
    @InjectMocks
    private AlrSendOfficialNoticeEmailHandlerFlowable handler;

    @Test
    void execute_shouldSendOfficialNotice() {
        String requestId = "REQ-12345";
        when(execution.getVariable(BpmnProcessConstants.REQUEST_ID)).thenReturn(requestId);
        handler.execute(execution);
        verify(alrOfficialNoticeService).sendOfficialNotice(requestId);
    }
    ...
    void execute_shouldPassNullRequestId_whenExecutionVariableMissing() {
        ...
        handler.execute(execution);
        verify(alrOfficialNoticeService).sendOfficialNotice(null);
    }
}
