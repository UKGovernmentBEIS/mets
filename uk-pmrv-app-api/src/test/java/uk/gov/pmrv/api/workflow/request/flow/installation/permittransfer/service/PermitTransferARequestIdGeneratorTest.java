...
class PermitTransferARequestIdGeneratorTest {
	...
	@Test
	void getTypes() {
		assertThat(generator.getTypes()).containsExactly(RequestType.PERMIT_TRANSFER_A);
	}
    
    @Test
    void getPrefix() {
    	assertThat(generator.getPrefix()).isEqualTo("AEMTA");
    }
    ...
    void generate() {
    	...
    	when(requestSequenceRepository.findByBusinessIdentifierAndType(String.valueOf(accountId), RequestType.PERMIT_TRANSFER_A))
    		.thenReturn(Optional.of(requestSequence));
    	
    	//invoke
		final String result = generator.generate(params);
    	
    	assertThat(result).isEqualTo("AEMTA" + "00001" + "-" + (currentSequence + 1));
