import { ChangeDetectionStrategy, Component, Inject } from '@angular/core';
import { UntypedFormGroup } from '@angular/forms';
import { ActivatedRoute, Router } from '@angular/router';

import { originalOrder } from '@shared/keyvalue-order';
import { DRE_TASK_FORM } from '@tasks/dre/core/dre-task-form.token';

import { PendingRequestService } from '../../../../core/guards/pending-request.service';
import { DreService } from '../../core/dre.service';
import { determinationReasonFormProvider } from './determination-reason-form.provider';
import { determinationReasonTypesLabelsMap } from './determination-reason-type-label.map';

@Component({
  selector: 'app-determination-reason',
  templateUrl: './determination-reason.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [determinationReasonFormProvider],
})
export class DeterminationReasonComponent {
  private readonly nextWizardStep = 'official-notice-reason';
  readonly originalOrder = originalOrder;
  determinationReasonTypesLabelsWithoutOtherMap = Object.keys(determinationReasonTypesLabelsMap)
    .filter((key) => key !== 'OTHER')
    .reduce((result, key) => ({ ...result, [key]: determinationReasonTypesLabelsMap[key] }), {});

  constructor(
    @Inject(DRE_TASK_FORM) readonly form: UntypedFormGroup,
    readonly dreService: DreService,
    private readonly router: Router,
    private readonly route: ActivatedRoute,
    private readonly pendingRequest: PendingRequestService,
  ) {}

  onSubmit(): void {
    if (!this.form.dirty) {
      this.router.navigate(['..', this.nextWizardStep], { relativeTo: this.route });
    } else {
      this.dreService
        .saveDre(
          {
            determinationReason: {
              ...this.form.value,
              supportingDocuments: this.form.value.supportingDocuments?.map((file) => file.uuid),
            },
          },
          false,
          {
            ...this.form.value.supportingDocuments?.reduce(
              (result, item) => ({ ...result, [item.uuid]: item.file.name }),
              {},
            ),
          },
        )
        .pipe(this.pendingRequest.trackRequest())
        .subscribe(() => this.router.navigate(['..', this.nextWizardStep], { relativeTo: this.route }));
    }
  }

  getBaseFileDownloadUrl() {
    return this.dreService.getBaseFileDownloadUrl();
  }
}
