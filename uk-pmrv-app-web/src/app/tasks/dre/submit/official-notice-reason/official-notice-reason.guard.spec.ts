...
import { OfficialNoticeReasonGuard } from './official-notice-reason.guard';

describe('OfficialNoticeReasonGuard', () => {
  let guard: OfficialNoticeReasonGuard;
  let router: Router;
  ...
  const routerStateSnapshot = {
    url: '/tasks/1/dre/submit/official-notice-reason',
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(OfficialNoticeReasonGuard);
  ...
  it('should redirect to first step if previous step not completed', async () => {
    store.setState({
      ...initialState,
      storeInitialized: true,
      isEditable: true,
  ...
  it('should activate when neither wizard not complete nor section completed', async () => {
    store.setState({
      ...initialState,
      storeInitialized: true,
      isEditable: true,
