...
import { InformationSourceGuard } from './information-source.guard';

describe('InformationSourceGuard', () => {
  let guard: InformationSourceGuard;
  let router: Router;
  ...
  const routerStateSnapshot = {
    url: '/tasks/1/dre/submit/information-sources/0',
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(InformationSourceGuard);
  ...
  it('should redirect to first step if index number is out of array list bound', async () => {
    store.setState({
      ...initialState,
      storeInitialized: true,
      isEditable: true,
    ...
    await expect(
      firstValueFrom(guard.canActivate(activatedRouteSnapshot, routerStateSnapshot) as Observable<true | UrlTree>),
    ).resolves.toEqual(router.parseUrl(`/tasks/1/dre/submit/information-sources`));
  });
  ...
  it('should activate when neither wizard is complete nor section completed', async () => {
    store.setState({
      ...
      storeInitialized: true,
      isEditable: true,
      requestTaskItem: updateMockedDre({ fee: undefined, informationSources: ['item1', 'item2'] }, false),
    });
