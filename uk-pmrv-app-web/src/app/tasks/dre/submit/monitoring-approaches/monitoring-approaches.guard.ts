...
export class MonitoringApproachesGuard implements CanActivate {
  ...
  private arePreviousStepsCompleted(payload: DreApplicationSubmitRequestTaskPayload) {
    return isDeterminationReasonStepCompleted(payload?.dre) && isOfficialNoticeReasonStepCompleted(payload?.dre);
  }
}
