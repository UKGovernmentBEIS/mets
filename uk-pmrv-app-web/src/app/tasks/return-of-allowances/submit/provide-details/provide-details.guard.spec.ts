...
import { ProvideDetailsGuard } from './provide-details.guard';

describe('ProvideDetailsGuard', () => {
  let guard: ProvideDetailsGuard;
  let router: Router;
  ...
  const routerStateSnapshot = {
    url: 'tasks/1/return-of-allowances/submit/provide-details',
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(ProvideDetailsGuard);
  ...
  it('should redirect to summary if section completed', async () => {
    ...
    ).resolves.toEqual(router.parseUrl(`/tasks/1/return-of-allowances/submit/summary`));
  });
  ...
  it('should activate when neither wizard is complete nor section completed', async () => {
    store.setState({
      ...
      storeInitialized: true,
      isEditable: true,
      requestTaskItem: updateMockedReturnOfAllowances({ numberOfAllowancesToBeReturned: undefined }, false),
    });
