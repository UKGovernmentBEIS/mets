...
import { updateMockedReturnOfAllowancesReturned } from '../../test/mock';
...
describe('SummaryGuard', () => {
  ...
  const routerStateSnapshot = {
    url: '/tasks/1/return-of-allowances/returned-allowances/summary',
  ...
  it('should return true if summary section completed', async () => {
    store.setState({
      ...
      isEditable: true,
      requestTaskItem: updateMockedReturnOfAllowancesReturned({}, true),
    });
  ...
  it('should redirect to first step if wizard not completed', async () => {
    store.setState({
      ...
      storeInitialized: true,
      isEditable: true,
      requestTaskItem: updateMockedReturnOfAllowancesReturned({ isAllowancesReturned: null as any }, false),
    });

    await expect(
      firstValueFrom(guard.canActivate(activatedRouteSnapshot, routerStateSnapshot) as Observable<true | UrlTree>),
    ).resolves.toEqual(router.parseUrl(`/tasks/1/return-of-allowances/returned-allowances/provide-returned-details`));
  });
