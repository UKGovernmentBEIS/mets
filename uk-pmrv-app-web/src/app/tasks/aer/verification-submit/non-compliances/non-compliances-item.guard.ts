...
export class NonCompliancesItemGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot): Observable<boolean | UrlTree> {
    return this.aerService.getPayload().pipe(
      ...
      map(
        (payload) =>
          ((payload as AerApplicationVerificationSubmitRequestTaskPayload).verificationReport.uncorrectedNonCompliances
          ...
          this.router.parseUrl(`/tasks/${route.paramMap.get('taskId')}/aer/verification-submit/non-compliances`),
      ),
    );
  }
}
