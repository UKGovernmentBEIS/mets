...
export const nonCompliancesItemFormProvider = {
  ...
  useFactory: (fb: UntypedFormBuilder, store: CommonTasksStore, route: ActivatedRoute) => {
    ...
    const item =
      index < nonCompliances?.uncorrectedNonCompliances?.length
        ? nonCompliances?.uncorrectedNonCompliances[index]
        : null;
    ...
    return fb.group({
      explanation: [
        ...
        {
          validators: [
            GovukValidators.required(noNonComplianceExplanation),
