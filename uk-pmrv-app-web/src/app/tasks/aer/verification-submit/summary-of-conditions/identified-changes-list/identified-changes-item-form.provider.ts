...
export const identifiedChangesItemFormProvider = {
  ...
  useFactory: (fb: UntypedFormBuilder, store: CommonTasksStore, route: ActivatedRoute) => {
    ...
    const item =
      index < summaryOfConditions.notReportedChanges?.length ? summaryOfConditions.notReportedChanges[index] : null;
    ...
    return fb.group({
      explanation: [
        ...
        {
          validators: [
            GovukValidators.required('Please provide details of the change'),
