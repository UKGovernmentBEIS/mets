...
import { identifiedChangesFormProvider } from '@tasks/aer/verification-submit/summary-of-conditions/identified-changes/identified-changes-form.provider';
...
export class IdentifiedChangesComponent {
              ...
              .postVerificationTaskSave(
                {
                  summaryOfConditions: {
                    ...
                    ...this.form.value,
                    ...(this.form.get('changesIdentified').value ? {} : { notReportedChanges: null }),
                  },
                },
                false,
                'summaryOfConditions',
  ...
  private navigateNext(summaryOfConditions: SummaryOfConditions) {
    if (this.form.get('changesIdentified').value) {
      return summaryOfConditions?.notReportedChanges?.length > 0
        ? this.router.navigate(['../identified-changes-list'], { relativeTo: this.route })
        : this.router.navigate(['../identified-changes-list', 0], { relativeTo: this.route });
    } else {
