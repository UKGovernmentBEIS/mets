...
@Component({
  selector: 'app-not-included-delete',
  template: `
...
export class NotIncludedDeleteComponent {
    ...
    map(
      ([paramMap, payload]) =>
        (payload as AerApplicationVerificationSubmitRequestTaskPayload).verificationReport.summaryOfConditions
          .approvedChangesNotIncluded[Number(paramMap.get('index'))],
    ),
          ...
          this.aerService.postVerificationTaskSave(
            {
              summaryOfConditions: {
                ...
                ).verificationReport.summaryOfConditions.approvedChangesNotIncluded
