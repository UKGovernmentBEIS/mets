...
@Component({
  ...
  template: `
    ...
    <app-compliance-ets-group
      ...
      [etsComplianceRules]="etsComplianceRules$ | async"></app-compliance-ets-group>
...
export class SummaryComponent implements OnInit, PendingRequest {
  ...
  ).pipe(map((payload) => payload.verificationReport.etsComplianceRules));
  ...
  onConfirm() {
    this.aerService
      .postVerificationTaskSave(null, true, 'etsComplianceRules')
