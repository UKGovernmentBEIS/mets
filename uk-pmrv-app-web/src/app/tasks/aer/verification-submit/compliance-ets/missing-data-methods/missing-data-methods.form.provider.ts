...
export const missingDataMethodsFormProvider = {
  ...
  useFactory: (fb: UntypedFormBuilder, store: CommonTasksStore) => {
    ...
    return fb.group({
      methodsApplyingMissingDataUsed: [
        { value: etsComplianceRules?.methodsApplyingMissingDataUsed ?? null, disabled: !state.isEditable },
        { validators: [GovukValidators.required(noSelection)] },
      ],
      methodsApplyingMissingDataNotUsedReason: [
        { value: etsComplianceRules?.methodsApplyingMissingDataNotUsedReason ?? null, disabled: !state.isEditable },
        {
