  ...
  canActivate(route: ActivatedRouteSnapshot): Observable<boolean | UrlTree> {
    return this.aerService.getPayload().pipe(
      ...
      map((payload) => {
        ...
        return (
          (uncorrectedNonConformities?.priorYearIssues &&
            uncorrectedNonConformities?.priorYearIssues.some((item, idx) => idx === index)) ||
          this.router.parseUrl(
            `/tasks/${route.paramMap.get('taskId')}/aer/verification-submit/non-conformities/previous-year/list`,
          )
        );
      }),
    );
