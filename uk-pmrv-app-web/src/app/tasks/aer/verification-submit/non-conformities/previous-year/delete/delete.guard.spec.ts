...
import { DeleteGuard } from '@tasks/aer/verification-submit/non-conformities/previous-year/delete/delete.guard';
...
describe('DeleteGuard', () => {
  ...
  it('should redirect to `list` when index is not found', async () => {
    ...
    await expect(
      firstValueFrom(guard.canActivate(activatedRouteSnapshot) as Observable<true | UrlTree>),
    ).resolves.toEqual(router.parseUrl('/tasks/1/aer/verification-submit/non-conformities/previous-year/list'));
  });

  it('should redirect to `list` when `priorYearIssues` is empty or null', async () => {
    activatedRouteSnapshot.params = { taskId: 1, index: 1 };
    store.setState(
      mockStateBuild({
        uncorrectedNonConformities: {
