      ...
      map((payload) => {
        ...
        return (
          (uncorrectedNonConformities?.uncorrectedNonConformities &&
            uncorrectedNonConformities?.uncorrectedNonConformities.some((item, idx) => idx === index)) ||
          this.router.parseUrl(`/tasks/${route.paramMap.get('taskId')}/aer/verification-submit/non-conformities/list`)
        );
      }),
    );
  }
