...
export class MisstatementsItemDeleteComponent implements PendingRequest {
    ...
    map(
      ([paramMap, payload]) =>
        (payload as AerApplicationVerificationSubmitRequestTaskPayload).verificationReport.uncorrectedMisstatements
          .uncorrectedMisstatements[Number(paramMap.get('index'))],
    ),
          ...
          this.aerService.postVerificationTaskSave(
            {
              uncorrectedMisstatements: {
                ...
                ).verificationReport.uncorrectedMisstatements.uncorrectedMisstatements
