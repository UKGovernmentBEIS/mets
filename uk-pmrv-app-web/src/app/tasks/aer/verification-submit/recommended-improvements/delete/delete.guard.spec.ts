...
describe('DeleteGuard', () => {
  ...
  it('should redirect to `list` when index is not found', async () => {
    ...
    await expect(
      firstValueFrom(guard.canActivate(activatedRouteSnapshot) as Observable<true | UrlTree>),
    ).resolves.toEqual(router.parseUrl('/tasks/1/aer/verification-submit/recommended-improvements/list'));
  });

  it('should redirect to `list` when `recommendedImprovements` is empty or null', async () => {
    activatedRouteSnapshot.params = { taskId: 1, index: 1 };
    store.setState(
      mockStateBuild({
        recommendedImprovements: {
