...
import { SourceStream, SourceStreamsService, TasksService } from 'pmrv-api';
...
describe('SourceStreamDetailsComponent', () => {
  ...
  const sourceStreamsService: MockType<SourceStreamsService> = {
    getSourceStreamCategories: jest.fn().mockReturnValue(
      of([
        {
  ...
  class Page extends BasePage<SourceStreamDetailsComponent> {
    ...
    set otherDescriptionName(value: SourceStream['otherDescriptionName']) {
      this.setInputValue('#otherDescriptionName', value);
    }
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: SourceStreamsService, useValue: sourceStreamsService },
      ],
    }).compileComponents();
  });
  ...
  describe('for adding new source stream', () => {
    ...
    it('should raise validation error for OTHER description', () => {
      ...
      page.otherDescriptionName = 'd'.repeat(301);
      fixture.detectChanges();
      page.submitButton.click();
      fixture.detectChanges();

      expect(page.errorSummary).toBeTruthy();
      expect(page.errorSummaryLinks).toEqual(['Enter up to 300 characters']);
    });
