...
describe('NaceCodeSubCategoryComponent', () => {
  ...
  const route = {
    snapshot: new ActivatedRouteSnapshotStub(null, { mainActivity: 'AGRICULTURE_FORESTRY_AND_FISHING' }),
  };
  ...
  it('should choose selected sub category and navigate to installation activity', () => {
    ...
    expect(navigateSpy).toHaveBeenCalledWith(['..', 'installation-activity'], {
      relativeTo: route,
      queryParams: { subCategory: 'CROP_AND_ANIMAL_PRODUCTION_HUNTING_AND_RELATED_SERVICE_ACTIVITIES' },
    });
  });
});
