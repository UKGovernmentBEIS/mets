...
export const industrialCrfCodeFormProvider = {
  ...
  useFactory: (fb: FormBuilder, store: CommonTasksStore, route: ActivatedRoute) => {
    ...
    const code: AerRegulatedActivity['industrialCrf'] = activity?.industrialCrf ?? null;
    ...
    const group = fb.group(
      {
        industrialCrfCategory: [category, GovukValidators.required('You must select at least one process')],
        industrialCrf: [code, GovukValidators.required('You must select at least one process')],
      },
      {
        updateOn: 'change',
      },
