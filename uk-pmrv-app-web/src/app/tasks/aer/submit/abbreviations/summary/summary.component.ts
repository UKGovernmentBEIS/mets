  ...
  template: `
    <app-aer-task [breadcrumb]="true">
      ...
      <app-abbreviations-summary-template
        ...
        [data]="abbreviations$ | async"></app-abbreviations-summary-template>
...
export class SummaryComponent implements PendingRequest {
  ...
  abbreviations$ = (this.aerService.getPayload() as Observable<AerApplicationSubmitRequestTaskPayload>).pipe(
    map((payload) => payload.aer?.abbreviations),
  );
  ...
  onSubmit(): void {
    this.aerService
      .postTaskSave(undefined, undefined, true, 'abbreviations')
