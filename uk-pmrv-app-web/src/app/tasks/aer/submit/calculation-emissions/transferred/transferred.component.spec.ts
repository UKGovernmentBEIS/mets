...
import { CalculationOfCO2Emissions, TasksService } from 'pmrv-api';
...
describe('TransferredComponent', () => {
  ...
  class Page extends BasePage<TransferredComponent> {
    get areEmissionsTransferredRadios() {
      return this.queryAll<HTMLInputElement>('input[name$="entryAccountingForTransfer"]');
    }
    get transferDirectionRadios() {
      return this.queryAll<HTMLInputElement>('input[name$="transferDirection"]');
    }
  ...
  describe('for adding a new source stream emission', () => {
    ...
    it('should submit a valid form, update the store and navigate to summary', () => {
      ...
      expect(page.errorSummaryLinks).toEqual(['Select a category']);

      page.areEmissionsTransferredRadios[0].click();
      fixture.detectChanges();

      page.transferDirectionRadios[0].click();
      fixture.detectChanges();
      ...
      expect(navigateSpy).toHaveBeenCalledWith(['../transferred-details'], { relativeTo: route });
    });
  });
  ...
  describe('for editing a source stream emission', () => {
    ...
    it('should  fill the form with data from the store', () => {
      ...
      expect(page.areEmissionsTransferredRadios.length).toEqual(2);
      expect(page.areEmissionsTransferredRadios[0].checked).toBeTruthy();
      expect(page.areEmissionsTransferredRadios[1].checked).toBeFalsy();
    });
    ...
    it('should post a valid form and navigate to next step ', () => {
      ...
      expect(tasksService.processRequestTaskAction).toHaveBeenCalledTimes(1);
      expect(navigateSpy).toHaveBeenCalledWith(['../transferred-details'], {
        relativeTo: TestBed.inject(ActivatedRoute),
      });
    });

    it('should edit form submit and navigate to next step ', () => {
      const navigateSpy = jest.spyOn(router, 'navigate');
      tasksService.processRequestTaskAction.mockReturnValueOnce(of({}));

      page.areEmissionsTransferredRadios[1].click();
      fixture.detectChanges();
            ...
            monitoringApproachEmissions: {
              ...
              CALCULATION_CO2: {
                ...
                sourceStreamEmissions: [
                  {
                    ...(
                      mockAerApplyPayload.aer.monitoringApproachEmissions.CALCULATION_CO2 as CalculationOfCO2Emissions
