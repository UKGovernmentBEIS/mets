...
export const operatorReportFormProvider = {
  ...
  useFactory: (fb: UntypedFormBuilder, store: CommonTasksStore, requestTaskFileService: RequestTaskFileService) => {
    ...
    return fb.group({
      ...
      areActivityLevelsEstimated: [
        { value: operatorActivityLevelReport?.areActivityLevelsEstimated ?? null, disabled: !state.isEditable },
        { validators: GovukValidators.required('Select yes or no') },
      ],
      comment: [
        { value: operatorActivityLevelReport?.comment ?? null, disabled },
        {
          validators: [
