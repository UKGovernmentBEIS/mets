...
import { mockCompletedNonComplianceNoticeOfIntentRequestTaskItem, updateMockedNoticeOfIntent } from '../../test/mock';
import { UploadNoticeOfIntentGuard } from './upload-notice-of-intent.guard';

describe('UploadNoticeOfIntentGuard', () => {
  let guard: UploadNoticeOfIntentGuard;
  let router: Router;
  ...
  const routerStateSnapshot = {
    url: 'tasks/1/non-compliance/notice-of-intent/upload-notice-of-intent',
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(UploadNoticeOfIntentGuard);
  ...
  it('should redirect to summary if section completed', async () => {
    ...
    ).resolves.toEqual(router.parseUrl(`/tasks/1/non-compliance/notice-of-intent/summary`));
  });
  ...
  it('should redirect to summary if wizard completed', async () => {
    store.setState({
      ...
      storeInitialized: true,
      isEditable: true,
      requestTaskItem: updateMockedNoticeOfIntent({}, false),
    });
  ...
  it('should activate when neither wizard is complete nor section completed', async () => {
    store.setState({
      ...
      storeInitialized: true,
      isEditable: true,
      requestTaskItem: updateMockedNoticeOfIntent({ noticeOfIntent: undefined }, false),
    });
