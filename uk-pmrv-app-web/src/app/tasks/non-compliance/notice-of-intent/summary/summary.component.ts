...
export class SummaryComponent implements OnInit {
  ...
  payload$ = this.nonComplianceService.getPayload().pipe(
    first(),
    map((payload) => payload as NonComplianceNoticeOfIntentRequestTaskPayload),
  );
  ...
  isTaskSubmitted$ = this.nonComplianceService.payload$.pipe(
    ...
    map(
      (payload) =>
        !(
          payload.payloadType === 'NON_COMPLIANCE_NOTICE_OF_INTENT_WAIT_FOR_PEER_REVIEW_PAYLOAD' ||
          payload.payloadType === 'NON_COMPLIANCE_NOTICE_OF_INTENT_APPLICATION_PEER_REVIEW_PAYLOAD'
        ),
    ),
  );
  ...
  onSubmit(): void {
    this.nonComplianceService
      .saveNoticeOfIntentSectionStatus(true)
