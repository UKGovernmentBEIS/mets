  ...
  canActivate(
    ...
    return this.store.pipe(
      map((storeState) => {
        ...
        const firstStep = `${state.url.slice(0, state.url.lastIndexOf('/'))}/upload-initial-notice`;
        return (
          !storeState.isEditable ||
          payload?.dailyPenaltyCompleted ||
          (payload?.dailyPenaltyNotice !== null && payload?.dailyPenaltyNotice !== undefined) ||
