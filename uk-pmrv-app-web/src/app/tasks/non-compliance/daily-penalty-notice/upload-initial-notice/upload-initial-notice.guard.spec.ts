...
import { UploadInitialNoticeGuard } from './upload-initial-notice.guard';

describe('UploadInitialNoticeGuard', () => {
  let guard: UploadInitialNoticeGuard;
  let router: Router;
  ...
  const routerStateSnapshot = {
    url: 'tasks/1/non-compliance/daily-penalty-notice/upload-initial-notice',
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(UploadInitialNoticeGuard);
  ...
  it('should redirect to summary if section completed', async () => {
    ...
    ).resolves.toEqual(router.parseUrl(`/tasks/1/non-compliance/daily-penalty-notice/summary`));
  });
  ...
  it('should activate when neither wizard is complete nor section completed', async () => {
    store.setState({
      ...
      storeInitialized: true,
      isEditable: true,
      requestTaskItem: updateMockedDailyPenaltyNotice({ dailyPenaltyNotice: undefined }, false),
    });
