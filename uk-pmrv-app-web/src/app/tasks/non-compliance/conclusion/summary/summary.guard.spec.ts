...
import { updateMockedConclusion } from '../../test/mock';
...
describe('SummaryGuard', () => {
  ...
  const routerStateSnapshot = {
    url: '/tasks/1/non-compliance/conclusion/summary',
  ...
  it('should return true if section is completed', async () => {
    store.setState({
      ...
      isEditable: true,
      requestTaskItem: updateMockedConclusion({}, true),
    });
  ...
  it('should redirect to first step if wizard not completed', async () => {
    store.setState({
      ...
      storeInitialized: true,
      isEditable: true,
      requestTaskItem: updateMockedConclusion({ complianceRestored: undefined }, false),
    });

    await expect(
      firstValueFrom(guard.canActivate(activatedRouteSnapshot, routerStateSnapshot) as Observable<true | UrlTree>),
    ).resolves.toEqual(router.parseUrl(`/tasks/1/non-compliance/conclusion/provide-conclusion`));
  });
});
