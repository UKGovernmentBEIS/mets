  ...
  canActivate(
    ...
    return this.store.pipe(
      map((storeState) => {
        ...
        const firstStep = `${state.url.slice(0, state.url.lastIndexOf('/'))}/upload-penalty-notice`;
        return (
          !storeState.isEditable ||
          payload?.civilPenaltyCompleted ||
          (payload?.civilPenalty !== null && payload?.civilPenalty !== undefined) ||
