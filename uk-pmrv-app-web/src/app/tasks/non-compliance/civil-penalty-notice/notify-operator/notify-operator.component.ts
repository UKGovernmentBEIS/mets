  ...
  template: `<div class="govuk-grid-row">
    <div class="govuk-grid-column-two-thirds">
      <app-notify-operator
        ...
        [confirmationMessage]="'Penalty notice sent to operator'"
    ...
    this.currentDomain$.pipe(takeUntil(this.destroy$)).subscribe((domain) => {
      if (domain === 'INSTALLATION') {
        this.route.paramMap.subscribe((paramMap) => {
          const link = ['/tasks', paramMap.get('taskId'), 'non-compliance', 'civil-penalty-notice'];
          this.breadcrumbService.show([{ text: 'Upload penalty notice: non-compliance', link }]);
        });
      }
    });
  }
