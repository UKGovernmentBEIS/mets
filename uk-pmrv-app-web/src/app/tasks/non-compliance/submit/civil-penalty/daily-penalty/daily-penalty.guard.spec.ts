...
import { DailyPenaltyGuard } from './daily-penalty.guard';

describe('DailyPenaltyGuard', () => {
  let guard: DailyPenaltyGuard;
  let router: Router;
  ...
  const routerStateSnapshot = {
    url: 'tasks/1/non-compliance/submit/daily-penalty',
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(DailyPenaltyGuard);
  ...
  it('should activate when neither wizard is complete nor section completed', async () => {
    store.setState({
      ...
      storeInitialized: true,
      isEditable: true,
      requestTaskItem: updateMockedNonCompliance({ dailyPenalty: undefined }, false),
    });
