...
import { NoticeOfIntentGuard } from './notice-of-intent.guard';

describe('NoticeOfIntentGuard', () => {
  let guard: NoticeOfIntentGuard;
  let router: Router;
  ...
  const routerStateSnapshot = {
    url: 'tasks/1/non-compliance/submit/notice-of-intent',
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(NoticeOfIntentGuard);
  ...
  it('should redirect to first step if details of breach not completed', async () => {
    store.setState({
      ...initialState,
      storeInitialized: true,
      isEditable: true,
  ...
  it('should activate when neither wizard is complete nor section completed', async () => {
    store.setState({
      ...
      storeInitialized: true,
      isEditable: true,
      requestTaskItem: updateMockedNonCompliance({ noticeOfIntent: undefined }, false),
    });
