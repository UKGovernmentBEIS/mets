...
import { mockStateBuild } from '@tasks/vir/comments-response/testing/mock-state';
...
describe('SummaryGuard', () => {
  ...
  it('should redirect to recommendation-response-review', async () => {
    ...
    await expect(
      firstValueFrom(guard.canActivate(activatedRouteSnapshot) as Observable<true | UrlTree>),
    ).resolves.toEqual(router.parseUrl('/tasks/1/vir/comments-response/B1/operator-followup'));
  });
});
