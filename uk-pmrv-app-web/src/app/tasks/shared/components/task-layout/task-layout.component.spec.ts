...
describe('TaskLayoutComponent', () => {
  ...
  class TestComponent {
    ...
    requestTaskItem$: Observable<RequestTaskItemDTO> = of({
  ...
  class Page extends BasePage<TestComponent> {
    ...
    get relatedTasks() {
      return this.queryAll('app-related-tasks .govuk-heading-s')?.map((el) => el.textContent.trim());
    }
    get TimelineTasks() {
      return this.queryAll('app-timeline-item .govuk-heading-s')?.map((el) => el.textContent.trim());
    }
  ...
  it('should show appropriate content', () => {
    ...
    expect(page.relatedTasks).toEqual(['Permit application returned to operator', 'Amend your permit application']);
