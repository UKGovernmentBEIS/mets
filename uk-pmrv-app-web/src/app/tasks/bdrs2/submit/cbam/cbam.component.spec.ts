...
import { CBAMComponent } from './cbam.component';

describe('CBAMComponent', () => {
  let page: Page;
  let router: Router;
  let store: CommonTasksStore;
  let activatedRoute: ActivatedRoute;
  let component: CBAMComponent;
  let fixture: ComponentFixture<CBAMComponent>;

  const tasksService = mockClass(TasksService);
  const route = new ActivatedRouteStub({ taskId: '1' });

  class Page extends BasePage<CBAMComponent> {
  ...
  const createComponent = () => {
    fixture = TestBed.createComponent(CBAMComponent);
  ...
  describe('for cbam details', () => {
    ...
    it('should display all HTMLElements and form with 0 errors', () => {
      ...
      expect(page.heading1.textContent.trim()).toEqual('UK Carbon Border Adjustment Mechanism (CBAM)');
    ...
    it('should display error on empty form submit', () => {
      ...
      expect(page.errorSummaryListContents).toEqual(['Select yes if additional sub-installation splits are required']);
    ...
    it('should submit a valid form and navigate to next page', async () => {
      ...
      expect(navigateSpy).toHaveBeenCalledWith(['../upload-report'], { relativeTo: activatedRoute });
    });
  });
});
