...
import { EmpStoreSideEffectsHandler } from './emp-uk-ets-store-side-effects.handler';
...
export class EmpUkEtsStoreDelegate extends EmpStoreDelegate {
  ...
  private sideEffectsHandler = new EmpStoreSideEffectsHandler(this.store);
  ...
  saveEmp(empTask: { [key in EmpTaskKey]?: EmpTask }, status: TaskItemStatus = 'complete'): Observable<any> {
    ...
    const payloadToUpdate = produce(payloadAfterSideEffects, (draft) => {
      ...
      if (
        ...
        (draft.payloadType === 'EMP_VARIATION_UKETS_APPLICATION_REVIEW_PAYLOAD' && taskKey !== 'empVariationDetails')
      ) {
  ...
  saveEmpOverallDecision(decision: EmpIssuanceDetermination & EmpVariationDetermination, sectionsCompleted: boolean) {
    const {
      requestTaskItem: { requestTask },
    } = this.store.getState();
