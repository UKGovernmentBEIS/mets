...
describe('VariationDetailsChangesComponent', () => {
  ...
  beforeEach(async () => {
    ...
    store.setState({
      requestTaskItem: {
        requestTask: {
          id: 19,
          type: 'EMP_VARIATION_CORSIA_APPLICATION_SUBMIT',
          assigneeFullName: 'TEST_ASSIGNEE',
          payload: {
            payloadType: 'EMP_VARIATION_CORSIA_APPLICATION_SUBMIT_PAYLOAD',
  ...
  it('should call the saveEmp function with the correct data when the form is valid', async () => {
    ...
    const data = {
      empVariationDetails: {
        ...store.empCorsiaDelegate.payload.empVariationDetails,
      },
    };
