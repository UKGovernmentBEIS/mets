...
export async function navigateToAddAircraftType(harness: RouterTestingHarness) {
  const el = screen.getByTestId('add-aicraft-btn');
...
export async function selectAircraftTypeFromSearch(
  ...
  expect(TestBed.inject(Location).path()).toEqual('/aircraft-type/search?change=true');
...
export async function fillAircraftTypeFormWithFUMM(harness: RouterTestingHarness) {
  ...
  const methodB = document.getElementById('fuelConsumptionMeasuringMethod-option1');
