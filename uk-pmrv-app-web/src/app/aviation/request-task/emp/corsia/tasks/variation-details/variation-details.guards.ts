...
import { empCorsiaQuery } from '../../../shared/emp-corsia.selectors';
...
export const canActivateVariationDetails: CanActivateFn = () => {
  ...
  return combineLatest([
    store.pipe(empCorsiaQuery.selectVariationDetails),
    store.pipe(empCorsiaQuery.selectVariationRegulatorLedReason),
  ]).pipe(
    take(1),
    tap(([variationDetails, variationRegulatorLedReason]) => {
      ...
      store.empCorsiaDelegate.setVariationDetails(variationDetails ?? null, variationRegulatorLedReason ?? null);
      formProvider.setFormValue(variationDetails, variationRegulatorLedReason);
    }),
    map(() => true),
  );
};
