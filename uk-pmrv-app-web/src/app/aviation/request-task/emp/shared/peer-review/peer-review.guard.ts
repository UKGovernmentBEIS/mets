...
import { peerReviewRequestTaskActionTypes } from '@aviation/request-task/util';

export const canActivateEmpPeerReview: CanActivateFn = (route: ActivatedRouteSnapshot) => {
  ...
  return store.pipe(
    ...
    map((payload) => {
      return (
        (allowedRequestTaskActions.some((action) => peerReviewRequestTaskActionTypes.includes(action)) &&
...
export const canActivateEmpVariationRegLedPeerReview: CanActivateFn = (route: ActivatedRouteSnapshot) => {
  const store = inject(RequestTaskStore);
  const allowedRequestTaskActions = store.getState().requestTaskItem.allowedRequestTaskActions;
