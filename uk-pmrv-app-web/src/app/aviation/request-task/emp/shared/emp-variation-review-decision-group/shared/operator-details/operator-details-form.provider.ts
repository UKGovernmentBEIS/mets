...
export interface OperatorDetailsFormModel {
  operatorName: FormGroup<Record<keyof EmpOperatorDetails, FormControl>>;
  flightIdentification: FormGroup<Record<keyof EmpOperatorDetails, FormControl>>;
  airOperatingCertificate: FormGroup<Record<keyof EmpOperatorDetails, FormControl>>;
  operatingLicense: FormGroup<Record<keyof EmpOperatorDetails, FormControl>>;
  organisationStructure: FormGroup<Record<keyof EmpOperatorDetails, FormControl>>;
  activitiesDescription: FormGroup<Record<keyof EmpOperatorDetails, FormControl>>;
}

@Injectable()
export class OperatorDetailsFormProvider implements TaskFormProvider<EmpOperatorDetails, OperatorDetailsFormModel> {
  ...
  setFormValue(formValues: EmpOperatorDetails | undefined): void {
    if (formValues) {
  ...
  private disableFlightIdentificationFormGroupConditionally() {
    const { flightIdentification } = this._form.value as EmpOperatorDetails;
  ...
  private disableAirOperatingCertificateFormGroupConditionally() {
    const { airOperatingCertificate } = this._form.value as EmpOperatorDetails;
  ...
  private disableOperatingLicenseFormGroupConditionally() {
    const { operatingLicense } = this._form.value as EmpOperatorDetails;
  ...
  private operatorNameCreateInitialForm() {
    return this.fb.group(
      {
        operatorName: new FormControl<EmpOperatorDetails['operatorName']>(null, {
          validators: [
        ...
        crcoCode: new FormControl<EmpOperatorDetails['crcoCode']>(null),
      },
      { updateOn: 'change' },
    ) as unknown as FormGroup<Record<keyof EmpOperatorDetails, FormControl>>;
  }
  ...
  private organisationStructureCreateInitialForm() {
    const partners = (
      this.store.empDelegate.payload.emissionsMonitoringPlan.operatorDetails
    ...
    return this.fb.group(
      {
        operatorType: new FormControl<ActivitiesDescription['operatorType']>(null, {
          validators: GovukValidators.required('Say whether you are a commercial or non-commercial operator'),
        }),
        flightTypes: new FormControl<ActivitiesDescription['flightTypes']>([], {
          validators: GovukValidators.required('Say if you operate scheduled or non-scheduled flights'),
        }),
        operationScopes: new FormControl<ActivitiesDescription['operationScopes']>([], {
          validators: GovukValidators.required('Say if your scope of operation is UK domestic or UK to EEA countries'),
        }),
        activityDescription: new FormControl<ActivitiesDescription['activityDescription']>(null, {
          validators: [
            GovukValidators.required('Say what kind of activities you carry out'),
