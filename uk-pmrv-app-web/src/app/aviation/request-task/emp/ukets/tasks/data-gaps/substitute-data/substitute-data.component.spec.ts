...
import { SubstituteDataComponent } from './substitute-data.component';
...
describe('Secondary data sources form', () => {
  ...
  beforeEach(async () => {
    ...
    await harness.navigateByUrl('/data-gaps/substitute-data', SubstituteDataComponent);
    harness.detectChanges();
  });
  it('should be in root location path', () => {
    expect(TestBed.inject(Location).path()).toEqual('/data-gaps/substitute-data');
  });
  it('should render substitute data form', () => {
    expect(screen.getByTestId('substitute-data')).toBeInTheDocument();
  });
  it('should populate form', async () => {
    const input = 'substitute-data desc';
    const textarea = document.getElementById('substituteData');
    await user.type(textarea, input);
    harness.detectChanges();
    await user.click(screen.getByText('Continue'));
    const req = httpTestingController.expectOne(basePath + '/v1.0/tasks/actions');
    expect(req.request.method).toEqual('POST');
    req.flush(createDataGapsFixture({ substituteData: input }));
    harness.detectChanges();
    ...
    expect(
      (store.getValue().requestTaskItem.requestTask.payload as EmpRequestTaskPayloadUkEts).emissionsMonitoringPlan
        .dataGaps.substituteData,
    ).toBeTruthy();
    expect(await screen.findByTestId('other-data-gaps-types')).toBeInTheDocument();
  });
});
