...
import { OtherDataGapsTypesComponent } from './other-data-gaps-types.component';
...
describe('Secondary data sources form', () => {
  ...
  beforeEach(async () => {
    ...
    await harness.navigateByUrl('/data-gaps/other-data-gaps-types', OtherDataGapsTypesComponent);
    harness.detectChanges();
  });
  it('should be in root location path', () => {
    expect(TestBed.inject(Location).path()).toEqual('/data-gaps/other-data-gaps-types');
  });
  it('should render other data gap types form', () => {
    expect(screen.getByTestId('other-data-gaps-types')).toBeInTheDocument();
  });
  it('should populate form', async () => {
    const input = 'other-data-gaps-types desc';
    const textarea = document.getElementById('otherDataGapsTypes');
    ...
    req.flush(
      createDataGapsFixture({
        dataGaps: 'asd',
        secondaryDataSources: 'asdasd',
        substituteData: 'asdasd',
        otherDataGapsTypes: input,
      }),
    );
    harness.detectChanges();
