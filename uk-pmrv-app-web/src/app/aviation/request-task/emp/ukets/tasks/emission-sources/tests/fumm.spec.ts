...
describe('Emission sources integration - FUMM', () => {
  ...
  it('should navigate to other form if any other fuel was selected', async () => {
    ...
    harness.detectChanges();
    expect(screen.getByTestId('aircraft-other-fuel-form')).toBeInTheDocument();
    const otherFuelTextArea = document.querySelector('textarea');
    await userEvent.type(otherFuelTextArea, 'some fuel description');
    await userEvent.click(screen.getByText('Continue'));
