...
@Component({
  ...
  imports: [SharedModule, ReturnToLinkComponent, IndependentReviewCorsiaTemplateComponent],
})
export class SummaryComponent {
  vm$: Observable<ViewModel> = combineLatest([
    this.store.pipe(requestTaskQuery.selectIsEditable),
    this.store.pipe(aerVerifyCorsiaQuery.selectStatusForTask('independentReview')),
  ]).pipe(
    map(([isEditable, taskStatus]) => {
      return {
        heading: 'Check your answers',
        independentReview: this.formProvider.form.valid ? this.formProvider.getFormValue() : null,
  ...
  constructor(
    @Inject(TASK_FORM_PROVIDER) private formProvider: IndependentReviewFormProvider,
  ...
  onSubmit() {
    (this.store.aerVerifyDelegate as AerVerifyCorsiaStoreDelegate)
      .saveAerVerify({ independentReview: this.formProvider.getFormValue() }, 'complete')
