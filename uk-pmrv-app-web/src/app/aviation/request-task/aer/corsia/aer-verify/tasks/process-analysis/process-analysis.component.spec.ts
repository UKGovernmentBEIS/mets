...
import { ProcessAnalysisComponent } from '@aviation/request-task/aer/corsia/aer-verify/tasks/process-analysis/process-analysis.component';
...
describe('ProcessAnalysisComponent', () => {
  ...
  class Page extends BasePage<ProcessAnalysisComponent> {
    ...
    set verificationActivities(value: string) {
      this.setInputValue('#verificationActivities', value);
    }
  ...
  describe('for new process analysis', () => {
    beforeEach(() => {
    ...
    it('should display error on empty form submit', () => {
      ...
      expect(page.errorSummaryListContents).toEqual(['Describe the verification activities and results']);
    });

    it('should submit a valid form and navigate to `strategic-analysis` page', () => {
      const navigateSpy = jest.spyOn(router, 'navigate');
      tasksService.processRequestTaskAction.mockReturnValueOnce(of({}));

      page.verificationActivities = 'My verification activities';
      page.submitButton.click();
      fixture.detectChanges();
      ...
      expect(navigateSpy).toHaveBeenCalledWith(['strategic-analysis'], { relativeTo: activatedRoute });
    });
  });

  describe('for existing process analysis', () => {
    beforeEach(() => {
      store = TestBed.inject(RequestTaskStore);
      formProvider = TestBed.inject<ProcessAnalysisFormProvider>(TASK_FORM_PROVIDER);
    ...
    it('should edit, submit a valid form and navigate to `strategic-analysis` page', () => {
      const navigateSpy = jest.spyOn(router, 'navigate');
      tasksService.processRequestTaskAction.mockReturnValueOnce(of({}));

      expect(page.errorSummary).toBeFalsy();

      page.verificationActivities = 'My verification activities changed';
      page.submitButton.click();
      fixture.detectChanges();
      ...
      expect(tasksService.processRequestTaskAction).toHaveBeenCalledWith({
        ...
        requestTaskActionPayload: {
          processAnalysis: {
            verificationActivities: 'My verification activities changed',
            strategicAnalysis: 'My strategic analysis',
            dataSampling: 'My data sampling',
            dataSamplingResults: 'My data sampling results',
            emissionsMonitoringPlanCompliance: 'My emissions monitoring plan compliance',
