...
export const canActivateAviationAerDataGaps: CanActivateFn = () => {
...
export const canActivateDataGapsList: CanActivateFn = (route) => {
  ...
  return formProvider.getFormValue().dataGaps?.length > 0 || addUrl;
};

export const canActivateDataGapEdit: CanActivateFn = (route) => {
  const index = +route.paramMap.get('index');
  const dataGaps = inject<DataGapsFormProvider>(TASK_FORM_PROVIDER).getFormValue()?.dataGaps;
