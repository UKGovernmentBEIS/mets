...
import { AviationOperatorDetails, IssuingAuthoritiesService } from 'pmrv-api';
...
export abstract class BaseOperatorDetailsComponent implements OnInit {
  ...
  getform(operatorDetailsSection: keyof AviationOperatorDetails) {
  ...
  submitForm(
    formControlName: keyof AviationOperatorDetails,
    operatorDetails: AviationOperatorDetails,
    navigateTo: string,
    ...
    if (formControlName ? this.getform(formControlName).valid : this.formProvider.form?.valid) {
