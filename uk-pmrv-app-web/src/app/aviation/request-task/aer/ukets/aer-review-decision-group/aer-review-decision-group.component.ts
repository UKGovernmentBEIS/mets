...
import { AerTaskKey, requestTaskQuery, RequestTaskStore } from '../../../store';
import { aerQuery } from '../../shared/aer.selectors';
import { aerReviewGroupMap, AerUkEtsReviewGroup } from '../../shared/util/aer.util';
...
export class AerReviewDecisionGroupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.groupKey = aerReviewGroupMap[this.taskKey];
  ...
  onSubmit() {
    if (this.form.valid) {
      ...
      (this.store.aerDelegate as AerStoreDelegate)
        .saveAerReviewDecision(this.getFormData(), this.taskKey)
