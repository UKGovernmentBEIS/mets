...
export class EmissionSourcesComponent {
  ...
  vm$: Observable<ViewModel & EmpSubmittedViewModel> = combineLatest([
    ...
    map(([payload, regulatorViewer, requestActionType]) => ({
      ...
      aircraftTypesToBeUsed: payload.emissionsMonitoringPlan.emissionSources.aircraftTypes
        .map(appendIndex)
        .filter(({ isCurrentlyUsed }) => !isCurrentlyUsed),
      fuelTypes: this.getFuelTypes(payload.emissionsMonitoringPlan.emissionSources),
      ...
      originalFuelTypes: payload.originalEmpContainer
        ? this.getFuelTypes(payload.originalEmpContainer.emissionsMonitoringPlan.emissionSources)
        : null,
  ...
  private getFuelTypes(emissionSources: EmpEmissionSources): { id: string; key: string; value: string }[] {
