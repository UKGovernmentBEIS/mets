...
import { EmpOperatorDetails, LimitedCompanyOrganisation } from 'pmrv-api';

interface ViewModel {
  data: EmpOperatorDetails;
...
export class OperatorDetailsComponent {
  ...
  vm$: Observable<ViewModel & EmpSubmittedViewModel> = combineLatest([
    ...
    map(([payload, regulatorViewer, requestActionType]) => ({
      data: payload.emissionsMonitoringPlan.operatorDetails,
      certificationFiles:
        payload.emissionsMonitoringPlan.operatorDetails.airOperatingCertificate?.certificateFiles?.map((uuid) => {
      ...
      originalCertificationFiles:
        payload.originalEmpContainer?.emissionsMonitoringPlan.operatorDetails.airOperatingCertificate?.certificateFiles?.map(
          (uuid) => {
