...
describe('AddComponent', () => {
  ...
  beforeEach(async () => {
    verifierUsersInvitationService = {
      inviteVerifierAdminUserByVerificationBodyId: jest.fn(),
    };
  ...
  it('should submit add a new contact', () => {
    verifierUsersInvitationService.inviteVerifierAdminUserByVerificationBodyId.mockReturnValue(asyncData(null));
    ...
    expect(verifierUsersInvitationService.inviteVerifierAdminUserByVerificationBodyId).toHaveBeenCalledWith(1, user);
  ...
  it('should require that the email is unique', () => {
    verifierUsersInvitationService.inviteVerifierAdminUserByVerificationBodyId.mockReturnValue(
    ...
    expect(page.errorSummary.querySelector('li').textContent.trim()).toEqual(
      'This user email already exists in the service',
    );
