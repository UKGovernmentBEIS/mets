...
describe('VerificationBodiesComponent', () => {
  ...
  beforeEach(async () => {
    verificationBodiesService = {
      getVerificationBodies: jest.fn().mockReturnValue(asyncData(mockVerificationBodies)),
      updateVerificationBodiesStatus: jest.fn().mockReturnValue(of(null)),
    };

    await TestBed.configureTestingModule({
      imports: [ReactiveFormsModule, GovukComponentsModule, SharedModule, RouterTestingModule, BusinessTestingModule],
  ...
  describe('for admin operators', () => {
    ...
    it('should update the verification body status only with the updated values on save', () => {
      ...
      expect(verificationBodiesService.updateVerificationBodiesStatus).toHaveBeenCalledWith([
        { id: 4, status: 'ACTIVE' },
      ]);
    });
    ...
    it('should display error page on business error of status update', async () => {
      verificationBodiesService.updateVerificationBodiesStatus.mockReturnValue(
      ...
      await expectBusinessErrorToBe(savePartiallyNotFoundVerificationBodyError);
    });
  });
