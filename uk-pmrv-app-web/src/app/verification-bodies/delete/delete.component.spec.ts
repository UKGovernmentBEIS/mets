...
describe('DeleteComponent', () => {
  ...
  const verificationBodiesService: MockType<VerificationBodiesService> = {
    deleteVerificationBodyById: jest.fn().mockReturnValue(of(null)),
  };
  ...
  it('should delete verification body on Confirm delete button click', () => {
    ...
    expect(verificationBodiesService.deleteVerificationBodyById).toHaveBeenCalledWith(1);
  });
  ...
  it('should show the business error page if the body has already been deleted', async () => {
    verificationBodiesService.deleteVerificationBodyById.mockReturnValue(
