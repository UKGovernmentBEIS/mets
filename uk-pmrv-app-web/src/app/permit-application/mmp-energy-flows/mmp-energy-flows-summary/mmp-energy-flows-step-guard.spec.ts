...
import { MMPEnergyFlowsStepGuard } from './mmp-energy-flows-step-guard';

describe('MMPEnergyFlowsStepGuard', () => {
  let guard: MMPEnergyFlowsStepGuard;
  let router: Router;
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(MMPEnergyFlowsStepGuard);
  ...
  it('should activate and go to summary if data model is complete', async () => {
    ...
    await expect(firstValueFrom(guard.canActivate(new ActivatedRouteSnapshotStub({ taskId: 1 })))).resolves.toEqual(
      router.parseUrl('/permit-issuance/1/mmp-energy-flows/summary'),
    );
  });
});
