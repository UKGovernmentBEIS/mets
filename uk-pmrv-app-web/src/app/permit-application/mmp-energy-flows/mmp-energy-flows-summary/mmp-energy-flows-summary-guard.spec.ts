...
import { MMPEnergyFlowsSummaryGuard } from './mmp-energy-flows-summary-guard';

describe('MMPEnergyFlowsSummaryGuard', () => {
  let guard: MMPEnergyFlowsSummaryGuard;
  let router: Router;
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(MMPEnergyFlowsSummaryGuard);
      ...
      mockStateBuild(
        {
          monitoringMethodologyPlans: {
            ...
            digitizedPlan: {
              energyFlows: energyFlowsDigitizedPlan.energyFlows,
            },
          },
        },
        {
  ...
  it('should activate if  data model is wrong', async () => {
    ...
    await expect(firstValueFrom(guard.canActivate(new ActivatedRouteSnapshotStub({ taskId: 1 })))).resolves.toEqual(
      router.parseUrl('/permit-issuance/1/mmp-energy-flows'),
    );
  });
});
