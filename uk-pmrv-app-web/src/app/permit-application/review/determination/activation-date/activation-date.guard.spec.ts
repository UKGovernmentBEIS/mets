...
import { ActivationDateGuard } from './activation-date.guard';

describe('ActivationDateGuard', () => {
  ...
  describe('permit issuance', () => {
    ...
    it('should redirect to determination if type is not GRANTED', async () => {
      jest.spyOn(router, 'getCurrentNavigation').mockReturnValue({ extras: { state: { changing: false } } } as any);
  ...
  describe('permit variation', () => {
    ...
    it('should return true if variation task and granted and activation date not exist', async () => {
      jest.spyOn(router, 'getCurrentNavigation').mockReturnValue({ extras: { state: { changing: false } } } as any);
