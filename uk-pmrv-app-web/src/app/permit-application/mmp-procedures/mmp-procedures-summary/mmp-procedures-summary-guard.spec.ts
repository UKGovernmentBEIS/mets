...
import { MMPProceduresSummaryGuard } from './mmp-procedures-summary-guard';

describe('MMPProceduresSummaryGuard', () => {
  let guard: MMPProceduresSummaryGuard;
  let router: Router;
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(MMPProceduresSummaryGuard);
  ...
  it('should activate if  data model is wrong', async () => {
    ...
    await expect(firstValueFrom(guard.canActivate(new ActivatedRouteSnapshotStub({ taskId: 1 })))).resolves.toEqual(
      router.parseUrl('/permit-issuance/1/mmp-procedures'),
    );
  });
});
