...
import { mockDigitizedPlanAnnualProductionLevel, mockDigitizedPlanWasteGasBalance } from '../testing/mock';
import { MMPSubInstallationsStepGuard } from './sub-installations-step-guard';

describe('MMPSubInstallationsStepGuard', () => {
  let guard: MMPSubInstallationsStepGuard;
  let router: Router;
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(MMPSubInstallationsStepGuard);
  ...
  it('should activate and go to summary if data model is complete', async () => {
    ...
    await expect(firstValueFrom(guard.canActivate(new ActivatedRouteSnapshotStub({ taskId: 1 })))).resolves.toEqual(
      router.parseUrl('/permit-issuance/1/mmp-sub-installations/0/summary'),
    );
  });
});
