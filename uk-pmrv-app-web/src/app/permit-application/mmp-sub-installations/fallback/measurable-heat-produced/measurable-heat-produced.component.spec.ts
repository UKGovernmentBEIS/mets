...
import {
  ...
  mockDigitizedPlanMeasurableHeatProduced,
...
import { MeasurableHeatProducedComponent } from './measurable-heat-produced.component';

describe('MeasurableHeatProducedComponent', () => {
  let page: Page;
  let store: PermitApplicationStore<PermitApplicationState>;
  let component: MeasurableHeatProducedComponent;
  let activatedRoute: ActivatedRoute;
  let router: Router;
  let fixture: ComponentFixture<MeasurableHeatProducedComponent>;

  const attachmentService = mockClass(RequestTaskAttachmentsHandlingService);
  const route = new ActivatedRouteStub({ subInstallationNo: '0' }, {}, { permitTask: 'monitoringMethodologyPlans' });

  class Page extends BasePage<MeasurableHeatProducedComponent> {
    get existsInputYes() {
      return this.query<HTMLInputElement>('#produced-option0');
    }

    get heatProducedValue() {
      return this.getInputValue('select[name="dataSources.0.heatProduced"]');
    }
    set heatProducedValue(value: string) {
      this.setInputValue('select[name="dataSources.0.heatProduced"]', value);
    }
  ...
  const createComponent = () => {
    fixture = TestBed.createComponent(MeasurableHeatProducedComponent);
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      declarations: [MeasurableHeatProducedComponent],
  ...
  describe('for existing sub installation', () => {
    ...
    it('should submit a valid form', () => {
      ...
      page.heatProducedValue = 'LEGAL_METROLOGICAL_CONTROL_READING';
      ...
      expect(navigateSpy).toHaveBeenCalledWith(['../measurable-heat-imported'], {
