...
import {
  ...
  mockDigitizedPlanFallbackDetailsFuel,
...
import { AnnualActivityLevelsFuelComponent } from './annual-activity-levels-fuel.component';

describe('AnnualActivityLevelsFuelComponent', () => {
  let page: Page;
  let store: PermitApplicationStore<PermitApplicationState>;
  let component: AnnualActivityLevelsFuelComponent;
  let activatedRoute: ActivatedRoute;
  let router: Router;
  let fixture: ComponentFixture<AnnualActivityLevelsFuelComponent>;

  const attachmentService = mockClass(RequestTaskAttachmentsHandlingService);
  const route = new ActivatedRouteStub({ subInstallationNo: '0' }, {}, { permitTask: 'monitoringMethodologyPlans' });

  class Page extends BasePage<AnnualActivityLevelsFuelComponent> {
    get fuelInputValue() {
      return this.getInputValue('select[name="fuelDataSources.0.fuelInput"]');
    }
    set fuelInputValue(value: string) {
      this.setInputValue('[name="fuelDataSources.0.fuelInput"]', value);
    }

    get energyContentValue() {
      return this.getInputValue('select[name="fuelDataSources.0.energyContent"]');
    }
    set energyContentValue(value: string) {
      this.setInputValue('[name="fuelDataSources.0.energyContent"]', value);
    }
  ...
  const createComponent = () => {
    fixture = TestBed.createComponent(AnnualActivityLevelsFuelComponent);
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      declarations: [AnnualActivityLevelsFuelComponent],
        ...
        mockStateBuild(
          {
            monitoringMethodologyPlans: {
              ...
              digitizedPlan: mockDigitizedPlanFallbackDetailsFuel,
            },
          },
          {
