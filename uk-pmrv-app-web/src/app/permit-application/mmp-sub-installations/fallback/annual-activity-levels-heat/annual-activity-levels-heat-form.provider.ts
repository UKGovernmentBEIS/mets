...
import { AnnualActivityHeatLevel } from 'pmrv-api';

export const annualActivityHeatAddFormFactory = {
  ...
  useFactory: (
    ...
    const measurableHeatFlowList = (fallbackApproach?.annualLevel as AnnualActivityHeatLevel)?.measurableHeatFlowList;
    const formGroup = fb.group(
      {
        measurableHeatFlowList: fb.array(
          measurableHeatFlowList?.length
            ? (fallbackApproach?.annualLevel as AnnualActivityHeatLevel)?.measurableHeatFlowList?.map(
                (dataSource, index) => {
                  return fb.group(
                    {
                      quantification: [
                        ...
                        ...(index === 0
                          ? [GovukValidators.required('Select the data source used for measurable heat flows')]
                          : []),
                      ],
                      net: [
                        dataSource?.net ?? '',
                        ...(index === 0
                          ? [GovukValidators.required('Select the data source used for net measurable heat flows')]
                          : []),
                      ],
                    },
                    index !== 0
            ...
            : [
                fb.group({
                  quantification: [
                    measurableHeatFlowList?.[0]?.quantification ?? '',
                    [GovukValidators.required('Select the data source used for measurable heat flows')],
                  ],
                  net: [
                    measurableHeatFlowList?.[0]?.net ?? '',
                    [GovukValidators.required('Select the data source used for net measurable heat flows')],
                  ],
                }),
              ],
