...
import { MMPSubInstallationsFallbackStepGuard } from './sub-installations-fallback-step-guard';

describe('MMPSubInstallationsFallbackStepGuard', () => {
  let guard: MMPSubInstallationsFallbackStepGuard;
  let router: Router;
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(MMPSubInstallationsFallbackStepGuard);
  ...
  it('should activate and go to summary if data model is complete', async () => {
    ...
    await expect(firstValueFrom(guard.canActivate(new ActivatedRouteSnapshotStub({ taskId: 1 })))).resolves.toEqual(
      router.parseUrl('/permit-issuance/1/mmp-sub-installations/fallback/0/summary'),
    );
  });
});
