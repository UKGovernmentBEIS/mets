...
import { VinylChlorideMonomerSP } from 'pmrv-api';

export const calculationVinylChlorideMonomerAddFormFactory = {
  ...
  useFactory: (
    ...
    const dataSources = (productBenchmark?.specialProduct as VinylChlorideMonomerSP)?.dataSources;
    const formGroup = fb.group(
      {
        dataSources: fb.array(
          dataSources?.length
            ? (productBenchmark?.specialProduct as VinylChlorideMonomerSP)?.dataSources?.map((dataSource) => {
                return fb.group({
                  detail: [
                    dataSource?.detail ?? '',
                    [
                      GovukValidators.required(
                        'Select the calculation method used for quantification of heat from hydrogen',
                      ),
                    ],
                  ],
                });
          ...
          [GovukValidators.required('Select the calculation method used for quantification of heat from hydrogen')],
        ),
