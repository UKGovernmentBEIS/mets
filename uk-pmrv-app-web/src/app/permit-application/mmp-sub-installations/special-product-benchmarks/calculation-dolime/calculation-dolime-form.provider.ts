...
import { DolimeSP } from 'pmrv-api';

export const calculationDolimeAddFormFactory = {
  ...
  useFactory: (
    ...
    const dataSources = (productBenchmark?.specialProduct as DolimeSP)?.dataSources;
    const formGroup = fb.group(
      {
        dataSources: fb.array(
          dataSources?.length
            ? (productBenchmark?.specialProduct as DolimeSP)?.dataSources?.map((dataSource) => {
                return fb.group({
