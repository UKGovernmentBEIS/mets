...
import { EthyleneOxideEthyleneGlycolsSP } from 'pmrv-api';

export const calculationEthyleneOxideGlycolsAddFormFactory = {
  ...
  useFactory: (
    ...
    const dataSources = (productBenchmark?.specialProduct as EthyleneOxideEthyleneGlycolsSP)?.dataSources;
          ...
          dataSources?.length
            ? (productBenchmark?.specialProduct as any)?.dataSources?.map((dataSource, index) => {
                if (index === 0) {
                  return fb.group({
                    details: fb.group({
                      ETHYLEN_OXIDE: [
                        dataSource?.details?.ETHYLEN_OXIDE ?? '',
                        [GovukValidators.required('Select the calculation method used for ethylene oxide')],
                      ],
                      MONOTHYLENE_GLYCOL: [
                        dataSource?.details?.MONOTHYLENE_GLYCOL ?? '',
                        [GovukValidators.required('Select the calculation method used for monoethylene glycol')],
                      ],
                      DIETHYLENE_GLYCOL: [
                        dataSource?.details?.DIETHYLENE_GLYCOL ?? '',
                        [GovukValidators.required('Select the calculation method used for diethylene glycol')],
                      ],
                      TRIETHYLENE_GLYCOL: [
                        dataSource?.details?.TRIETHYLENE_GLYCOL ?? '',
                        [GovukValidators.required('Select the calculation method used for triethylene glycol')],
                      ],
                    }),
                  });
                } else {
                  return fb.group({
                    details: fb.group(
                      {
                        ETHYLEN_OXIDE: [dataSource?.details?.ETHYLEN_OXIDE ?? ''],
                        MONOTHYLENE_GLYCOL: [dataSource?.details?.MONOTHYLENE_GLYCOL ?? ''],
                        DIETHYLENE_GLYCOL: [dataSource?.details?.DIETHYLENE_GLYCOL ?? ''],
                        TRIETHYLENE_GLYCOL: [dataSource?.details?.TRIETHYLENE_GLYCOL ?? ''],
                      },
                      {
                        validators: [
            ...
            : [
                fb.group({
                  details: fb.group({
                    ETHYLEN_OXIDE: [
                      dataSources?.[0]?.details?.ETHYLEN_OXIDE ?? '',
                      [GovukValidators.required('Select the calculation method used for ethylene oxide')],
                    ],
                    MONOTHYLENE_GLYCOL: [
                      dataSources?.[0]?.details?.MONOTHYLENE_GLYCOL ?? '',
                      [GovukValidators.required('Select the calculation method used for monoethylene glycol')],
                    ],
                    DIETHYLENE_GLYCOL: [
                      dataSources?.[0]?.details?.DIETHYLENE_GLYCOL ?? '',
                      [GovukValidators.required('Select the calculation method used for diethylene glycol')],
                    ],
                    TRIETHYLENE_GLYCOL: [
                      dataSources?.[0]?.details?.TRIETHYLENE_GLYCOL ?? '',
                      [GovukValidators.required('Select the calculation method used for triethylene glycol')],
                    ],
                  }),
                }),
