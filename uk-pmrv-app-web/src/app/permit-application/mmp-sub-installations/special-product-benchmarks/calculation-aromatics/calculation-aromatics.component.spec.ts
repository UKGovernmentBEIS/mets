...
import { mockDigitizedPlanSPAromatics, mockDigitizedPlanWasteGasBalance } from '../../testing/mock';
import { CalculationAromaticsComponent } from './calculation-aromatics.component';

describe('CalculationAromaticsComponent', () => {
  let page: Page;
  let store: PermitApplicationStore<PermitApplicationState>;
  let component: CalculationAromaticsComponent;
  let activatedRoute: ActivatedRoute;
  let router: Router;
  let fixture: ComponentFixture<CalculationAromaticsComponent>;

  const attachmentService = mockClass(RequestTaskAttachmentsHandlingService);
  const route = new ActivatedRouteStub({ subInstallationNo: '0' }, {}, { permitTask: 'monitoringMethodologyPlans' });

  class Page extends BasePage<CalculationAromaticsComponent> {
    get aromaticsFunctions() {
      return this.query<HTMLInputElement>('#relevantCWTFunctions');
    }
    ...
    get dataSourcesValue1() {
      return this.getInputValue('select[name="dataSources.0.NAPHTHA_GASOLINE_HYDROTREATER"]');
    }
    set dataSourcesValue1(value: string) {
      this.setInputValue('select[name="dataSources.0.NAPHTHA_GASOLINE_HYDROTREATER"]', value);
    }
  ...
  const createComponent = () => {
    fixture = TestBed.createComponent(CalculationAromaticsComponent);
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      declarations: [CalculationAromaticsComponent],
  ...
  describe('for existing sub installation', () => {
    ...
    it('should raise an error if the first dataSource is empty', () => {
      ...
      expect(page.checkboxes.length).toEqual(8);
      fixture.detectChanges();
