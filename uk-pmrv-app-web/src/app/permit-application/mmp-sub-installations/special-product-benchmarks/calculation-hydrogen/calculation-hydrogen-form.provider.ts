...
import { HydrogenSP } from 'pmrv-api';

export const calculationHydrogenAddFormFactory = {
  ...
  useFactory: (
    ...
    const dataSources = (productBenchmark?.specialProduct as HydrogenSP)?.dataSources;
    const formGroup = fb.group(
      {
        dataSources: fb.array(
          dataSources?.length
            ? (productBenchmark?.specialProduct as HydrogenSP)?.dataSources?.map((dataSource, index) => {
                return fb.group({
                  details: fb.group(
                    {
                      HYDROGEN_TOTAL_PRODUCTION: [
                        dataSource?.details?.HYDROGEN_TOTAL_PRODUCTION ?? '',
                        index === 0
                          ? [
                              GovukValidators.required(
                                'Select the calculation method used for total hydrogen production',
                              ),
                            ]
                          : [],
                      ],
                      HYDROGEN_VOLUME_FRACTION: [
                        dataSource?.details?.HYDROGEN_VOLUME_FRACTION ?? '',
                        index === 0
            ...
            : [
                fb.group({
                  details: fb.group({
                    HYDROGEN_TOTAL_PRODUCTION: [
                      dataSources?.[0]?.details?.HYDROGEN_TOTAL_PRODUCTION ?? '',
                      [GovukValidators.required('Select the calculation method used for total hydrogen production')],
                    ],
                    HYDROGEN_VOLUME_FRACTION: [
                      dataSources?.[0]?.details?.HYDROGEN_VOLUME_FRACTION ?? '',
                      [GovukValidators.required('Select the calculation method used for the volume fraction')],
                    ],
                  }),
                }),
