...
describe('EmissionFactorGuard', () => {
  ...
  it('should redirect to summary if task is completed', async () => {
    ...
    store.setState(mockStateBuild(undefined, { CALCULATION_PFC_Tier2EmissionFactor: [true] }));

    await expect(
      firstValueFrom(guard.canActivate(activatedRouteSnapshot) as Observable<true | UrlTree>),
    ).resolves.toEqual(router.parseUrl(`/permit-issuance/276/pfc/emission-factor/summary`));
  });
  ...
  it('should redirect to answers if wizard is completed', async () => {
    ...
    await expect(
      firstValueFrom(guard.canActivate(activatedRouteSnapshot) as Observable<true | UrlTree>),
    ).resolves.toEqual(router.parseUrl(`/permit-issuance/276/pfc/emission-factor/answers`));
