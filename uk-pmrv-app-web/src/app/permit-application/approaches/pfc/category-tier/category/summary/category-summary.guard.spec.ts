...
describe('CategorySummaryGuard', () => {
  ...
  it('should redirect to PFC source stream category view', async () => {
    store.setState(mockStateBuild(mockState));

    await expect(
      firstValueFrom(guard.canActivate(new ActivatedRouteSnapshotStub({ taskId: '237', index: '0' }))),
    ).resolves.toEqual(TestBed.inject(Router).parseUrl('/permit-issuance/237/pfc/category-tier/0/category'));
  });
});
