...
describe('CategorySummaryGuard', () => {
  ...
  it('should redirect to CALCULATION_CO2 source stream category view', async () => {
    store.setState(mockStateBuild(mockState));

    await expect(
      firstValueFrom(guard.canActivate(new ActivatedRouteSnapshotStub({ taskId: '237', index: '0' }))),
    ).resolves.toEqual(TestBed.inject(Router).parseUrl('/permit-issuance/237/calculation/category-tier/0/category'));
  });
});
