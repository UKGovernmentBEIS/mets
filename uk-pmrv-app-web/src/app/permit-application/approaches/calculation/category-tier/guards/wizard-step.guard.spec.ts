...
describe('WizardStepGuard', () => {
  ...
  activatedRouteSnapshot.data = { statusKey: 'CALCULATION_CO2_Calorific' };
  ...
  it('should redirect to answers page if wizard is complete', async () => {
    ...
    await expect(
      ...
      router.parseUrl(
        `/permit-issuance/${activatedRouteSnapshot.params.taskId}/calculation/category-tier/${activatedRouteSnapshot.params.index}/calorific-value/answers`,
      ),
    );
  });
});
