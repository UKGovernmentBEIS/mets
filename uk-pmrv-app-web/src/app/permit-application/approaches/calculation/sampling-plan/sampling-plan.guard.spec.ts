...
import { SamplingPlanGuard } from './sampling-plan.guard';

describe('SamplingPlanGuard', () => {
  let router: Router;
  let guard: SamplingPlanGuard;
  let store: PermitApplicationStore<PermitApplicationState>;

  const tasksService = mockClass(TasksService);
  const activatedRouteSnapshot = new ActivatedRouteSnapshot();
  activatedRouteSnapshot.url = [new UrlSegment('plan', null)];
  const routerStateSnapshot = {
    url: '/permit-issuance/276/calculation/sampling-plan',
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(SamplingPlanGuard);
