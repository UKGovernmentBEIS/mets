          ...
          return (
            (permitState.permitSectionsCompleted?.CALCULATION_CO2_Plan?.[0] &&
              this.router.parseUrl(
                state.url.slice(0, state.url.lastIndexOf(route.url[route.url.length - 1].path)).concat('summary'),
              )) ||
            (plan?.exist === undefined &&
              this.router.parseUrl(
                state.url.slice(0, state.url.lastIndexOf(route.url[route.url.length - 1].path) - 1),
              )) ||
            (plan?.exist === true &&
            ...
            this.router.parseUrl(
              state.url.slice(0, state.url.lastIndexOf(route.url[route.url.length - 1].path)).concat('answers'),
            )
          );
        }),
      )
