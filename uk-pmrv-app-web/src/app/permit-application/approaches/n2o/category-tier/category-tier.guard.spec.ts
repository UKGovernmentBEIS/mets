...
describe('CategoryTierGuard', () => {
  ...
  it('should activate emission point category view', async () => {
  ...
  it('should redirect to MEASUREMENT_N2O view', async () => {
    store.setState(mockState);

    await expect(
      firstValueFrom(guard.canActivate(new ActivatedRouteSnapshotStub({ taskId: '237', index: '2' }))),
    ).resolves.toEqual(TestBed.inject(Router).parseUrl('/permit-issuance/237/nitrous-oxide'));
