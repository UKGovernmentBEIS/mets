...
describe('SummaryComponent', () => {
  ...
  const applicationPayload: SubmitApplicationActionPayload = {
    ...
    legalEntity: {
      ...
      holdingCompany: { ...operator.detailsGroup.holdingCompanyGroup },
    },
  ...
  const state: InstallationAccountApplicationState = {
    tasks: [
      ...
      {
        ...initialState.tasks.find((task) => task.type === ApplicationSectionType.applicationType),
        value: applicationType,
      } as ApplicationTypeSection,
    ],
  };
  ...
  beforeEach(async () => {
    tasksService = { processRequestTaskAction: jest.fn().mockReturnValue(of(null)) };
  ...
  it('should display the contact details of the operator', () => {
    expect(getSummaryArrayByIndex(1)).toEqual(
      expect.arrayContaining([
        ...
        ['Country', 'Greece'],
        ['Post code', '26555'],
        ['Holding company name', 'TEST_HOLDING_COMPANY'],
        ['Holding company registration number', 'TEST_REG_NUM'],
        ['Holding company address', 'TEST_ADDR_L1   TEST_CITYTEST_POSTCODE'],
      ]),
    );
  });
