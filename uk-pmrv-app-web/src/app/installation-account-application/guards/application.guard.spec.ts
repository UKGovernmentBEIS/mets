...
import { InstallationAccountsService, LegalEntitiesService, RequestActionsService, TasksService } from 'pmrv-api';
...
describe('ApplicationGuard', () => {
  ...
  taskService.getTaskItemInfoById.mockReturnValue(of({ requestTask: { payload } } as any));
  ...
  const requestActionsService: MockType<RequestActionsService> = {
    getRequestActionById: jest.fn().mockReturnValue(of(response)),
  };
