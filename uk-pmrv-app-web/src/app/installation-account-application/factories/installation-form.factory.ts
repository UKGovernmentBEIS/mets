...
export const INSTALLATION_FORM = new InjectionToken<UntypedFormGroup>('Installation form');

export const installationFormFactory = {
  provide: INSTALLATION_FORM,
  useFactory: (fb: UntypedFormBuilder, accountsService: InstallationAccountsService) =>
    fb.group({
  ...
  deps: [UntypedFormBuilder, InstallationAccountsService],
};
...
function installationNameNotExists(accountsService: InstallationAccountsService): AsyncValidatorFn {
  return (control: AbstractControl): Observable<ValidationErrors | null> =>
    accountsService
      .isExistingAccountName(control.value)
      .pipe(
        map((res) =>
