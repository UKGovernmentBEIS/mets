...
import { AllowancesNumberGuard } from './allowances-number.guard';

describe('AllowancesNumberGuard', () => {
  let router: Router;
  let guard: AllowancesNumberGuard;
  let store: PermitSurrenderStore;

  const tasksService: MockType<TasksService> = {};

  const activatedRouteSnapshot = new ActivatedRouteSnapshot();
  activatedRouteSnapshot.url = [new UrlSegment('allowances-number', null)];
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(AllowancesNumberGuard);
  ...
  it('should redirect to first step when allowancesSurrenderRequired is not true', async () => {
    store.setState({
      ...mockTaskState,
      cessationCompleted: false,
