...
describe('UploadDocumentsGuard', () => {
  let router: Router;
  let guard: UploadDocumentsGuard;
  let store: PermitSurrenderStore;

  const tasksService: MockType<TasksService> = {};

  const activatedRouteSnapshot = new ActivatedRouteSnapshot();
  activatedRouteSnapshot.url = [new UrlSegment('upload-documents', null)];
  activatedRouteSnapshot.params = { taskId: 1 };
  activatedRouteSnapshot.data = { statusKey: 'SURRENDER_APPLY' };

  const routerStateSnapshot = {
    url: '/permit-surrender/1/apply/upload-documents',
  ...
  beforeEach(() => {
    ...
    guard = TestBed.inject(UploadDocumentsGuard);
