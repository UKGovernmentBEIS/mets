    ...
    <form (ngSubmit)="onSubmit()" *ngIf="form$ | async as form" [formGroup]="form">
      ...
      <div class="govuk-grid-row">
        <div class="govuk-grid-column-one-half">
          <div formControlName="faStatus" govuk-radio>
...
export class FaStatusComponent implements OnInit {
  ...
  form$ = this.account$.pipe(
    map((account) =>
      this.fb.group({
        faStatus: [(account as InstallationAccountDTO)?.faStatus, [GovukValidators.required('Select yes or no')]],
      }),
    ),
        ...
        switchMap(([form, account]) =>
          form.dirty
            ? this.accountUpdateService
                .updateFaStatus(account.id, {
                  faStatus: form.get('faStatus').value,
                })
                .pipe(tap((this.route.snapshot.data.accountPermit.account.faStatus = form.get('faStatus').value)))
            : of(null),
        ),
      )
