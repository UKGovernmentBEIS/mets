import { PermitRevocation } from 'pmrv-api';

export const permitRevocationMapper: Record<
  keyof PermitRevocation,
  { label: string; url: string; step: number; prefix?: string; error?: string }
> = {
  reason: {
    label: 'Reason for revoking permit',
    url: 'reason',
    step: 1,
  },
  stoppedDate: {
    label: 'Have the regulated activities at the installation stopped?',
    url: 'stop-date',
    error: 'invalidStoppedDate',
    step: 2,
  },
  activitiesStopped: {
    label: 'Have the regulated activities at the installation stopped?',
    url: 'stop-date',
    step: 2,
  },
  effectiveDate: {
    label: 'Effective date of notice',
    url: 'notice',
    error: 'invalidEffectiveDate',
    step: 3,
  },
  annualEmissionsReportRequired: {
    label: 'Annual emissions of monitoring revocation report required',
    url: 'report',
    prefix: 'Required by ',
    step: 4,
  },
  annualEmissionsReportDate: {
    label: 'Annual emissions of monitoring revocation report required',
    url: 'report',
    prefix: 'Required by ',
    error: 'invalidAnnualEmissionsReportDate',
    step: 4,
  },
  alrRequired: {
    label: 'Activity level report required',
    url: 'final-alr',
    prefix: 'Required by ',
    step: 5,
  },
  alrReportDate: {
    label: 'Activity level report required',
    url: 'final-alr',
    prefix: 'Required by ',
    error: 'invalidAlrRequiredDate',
    step: 5,
  },
  surrenderRequired: {
    label: 'Surrender of allowances required',
    url: 'surrender-allowances',
    prefix: 'Required by ',
    step: 6,
  },
  surrenderDate: {
    label: 'Surrender of allowances required',
    url: 'surrender-allowances',
    prefix: 'Required by ',
    error: 'invalidSurrenderDate',
    step: 6,
  },
  feeCharged: {
    label: 'Operator fee required',
    url: 'fee',
    error: 'invalidFeeDate',
    step: 7,
  },
  feeDate: {
    label: 'Operator fee required',
    url: 'fee',
    error: 'invalidFeeDate',
    step: 7,
  },
  feeDetails: {
    label: 'Operator fee required',
    url: 'fee',
    error: 'invalidFeeDate',
    step: 7,
  },
};
